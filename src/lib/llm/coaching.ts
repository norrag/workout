...
- effort_assignment present means a PERSON — the lifter or their coach — set this exercise's effort for this week; the program only priced the weight to meet it. Never explain that effort level as a program decision, never argue with it, and never treat a backed_off assignment as a decline in strength. Its reason is that person's own words: honor it, don't reinterpret it. If measured is false the work is deliberately light and is not being read as a strength measurement — that is intended, not a problem to flag.
