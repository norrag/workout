-- 20260705000001 — engine_params v19: hold-week reprice-down (R24 remainder)
--
-- The owner's 2026-07-02 R24 concern: on a week where the RIR ramp holds (the
-- default 3→2→2→1 ramp holds at week 2→3), the Option-A climb still advanced
-- targetReps by +1, so the load repriced DOWN — a "−5 lb, +1 rep" lateral move
-- mid-meso that reads as regression. Separately, because the recency anchor
-- decays between sessions, an identical "hold" priced a hair lower in week N+1
-- than week N — most concerning on cut/maintain blocks meant to preserve
-- strength.
--
-- Two deltas from v18, both `.optional()` (v18/earlier rows byte-identical,
-- hash/replay/fingerprint untouched; ABSENT ⇒ prior behavior):
--   climb_requires_rir_step   the +1 rep climb runs only on a week the target
--                             RIR actually stepped down (doc 13 §9.2's own
--                             premise); topping the window still resets/steps
--                             the load unconditionally (doc 10 double
--                             progression), and with climb_on_performed_reps
--                             demonstrated extra reps still advance the climb.
--   hold_week_anchor_deadband on a pure hold (no RIR step, no rep advance, no
--                             top-out) an anchor-drift shortfall under one
--                             loadable step is absorbed (hold the handled
--                             load); a full step or more is real signal and
--                             passes through.
--
-- Shipped INACTIVE (same discipline as v11–v17): v19 changes live hold-week
-- prescriptions, so activate manually only AFTER a replay_decisions diff
-- (docs/deployment/manual-operations.md). schema_version stays 5 (optional
-- fields are not a shape bump). The JSON below is the full materialization from
