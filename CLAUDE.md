...
- `docs/14-prescription-invalidation.md` — **design for the represcribe/invalidation mechanism** (per-user, per-exercise tunables such as the editable weight increment): generalizes the `params_version` staleness gate, defines the `NULL`-marker invalidation primitive and the override-aware recompute path. Read before building any feature that changes an engine input the user controls.
