...
- `docs/20-measure-companion-app.md` — **direction doc for MEASURE**, the companion measurement app (2026-07-25, N66): body weight tracking with read-time smoothing, circumference measurements, DEXA, import/export, and an integrated summary — sharing this repo's DB, design system, and MCP connector, with its own app shell. Not yet a build spec: it fixes the division of labour with WORKOUT, the topology decision (a `(measure)` route group + its own manifest inside this deployable, §3), the schema direction (§4), the pure `src/lib/measure/` module (§5), the cross-app seam (§7), and the open owner decisions (§13). **Phase 0 is a mockup pass and it gates every later phase** (hard rule 8). Read before any measurement-app work; doc 15 keeps authority over the BodySpec integration internals and doc 17 over macro goals.
- `docs/21-exercise-level-rir.md` — **authoritative build spec for exercise-level RIR** (2026-07-31, N67): a target RIR assigned per day-slot exercise per week, with **absolute** semantics (set wins, unset yields to the ramp), the load repriced off the anchor at **window-centered reps** — the deload's own mechanic at exercise grain — plus a per-exercise set cap, an earn-gate predicate, and an intent-keyed stats policy. Its **Phase 1 amends the doc-11 RIR premise** (N68/N38): logged sets capture `rir_reported`, and one resolution `rir_reported ?? target_rir` is used by the stamp, the anchor, and the compliance marker — closing the defect where every stats surface treated every set as taken to failure (a one-time upward re-levelling of historical e1RM). Consolidates the assessment thread (`docs/reviews/2026-07-31-exercise-level-rir.md`, plus the closed coach-override review); where they conflict, 21 wins; doc 16 keeps authority over progression internals, doc 14 over freshness, doc 10 over metric definitions. Read before touching per-set RIR, `target_rir` resolution, the deload/repricing path, or any stats surface that reads a per-set e1RM; phases in its §10.
