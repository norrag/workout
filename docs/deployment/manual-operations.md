...
| ① new | [Apply the MCP catalog-freshness migration (N91)](#apply-the-mcp-catalog-freshness-migration-n91) | Lands with the N91 connector PR. Not a break-on-deploy migration — the read path fails open, so shipping ahead of it degrades to the old behavior rather than warning everyone. But until it is applied the feature does nothing at all, and the owner's test loop (deploy → notice → Refresh → no notice) cannot run. |
...
### Apply the MCP catalog-freshness migration (N91)
...
| Step | What / why |
|---|---|
| **① Apply `20260912000001` to hosted** | Supabase MCP `apply_migration`. Creates one table with RLS enabled, an owner/admin `select` policy and no user write policy (writes are service-role, like `mcp_write_audit`). Additive — no existing table, column, policy or row is touched. Reversible with `drop table public.mcp_client_catalog;`. |
| **② Confirm it is being written** | After the deploy, open a connected assistant and ask it anything. `select * from public.mcp_client_catalog` should show one row per connection that has fetched a catalog since. A connection that has *not* refreshed will have no row until it is notified, then a row with a **null** `catalog_fingerprint` — that is the pre-tracking baseline, not a bug. |
