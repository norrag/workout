...
| Set **`NOTES_REPO_TOKEN`** (notes MCP tools, N67 / doc 05 §Notes area) | Project → Settings → Environment Variables (Production; Preview only if you want to exercise the tools from a preview deploy) | **Required before the notes tools do anything** — without it every `*_notes*` tool returns a config error naming this var, and the rest of the connector is unaffected. It lets `/api/mcp` commit to `docs/notes/**` in this repo, which is what makes a note captured from a phone land in `backlog.md` with no sync step. Steps: (1) GitHub → Settings → Developer settings → **Fine-grained personal access tokens** → Generate new token; (2) Resource owner `norrag`, **Only select repositories → `norrag/workout`**; (3) Repository permissions → **Contents: Read and write** (nothing else — no Actions, no Secrets, no Workflows); (4) set an expiry you're willing to rotate (90 days is fine — expiry surfaces as a clear GitHub 401 in the tool result); (5) paste as `NOTES_REPO_TOKEN`, redeploy, then call `get_notes_manual` from the connector to confirm. Optional `NOTES_REPO` (default `norrag/workout`) and `NOTES_REPO_BRANCH` (default `main`) override the target. Scope is enforced server-side too: only `docs/notes/**.md` paths, admin sessions only, one commit per call, and a concurrent write to the branch is rejected rather than clobbered. |
