...
- [x] Error handling/observability: structured logging, Sentry (or equivalent), Supabase log review. *(2026-07-02 / R20: one `reportError()` funnel (`src/lib/observability/`) — structured console floor always, dependency-free Sentry envelope delivery when `SENTRY_DSN` is set; `instrumentation.ts` `onRequestError` global server capture; root `global-error.tsx` + `(auth)` boundary + `/api/client-error` intake; deliberate degrade-gracefully catches (reconcile, week generation, seed decisions, MCP tool guard) all report before swallowing. **External:** set `SENTRY_DSN` in Vercel — see manual-operations; Supabase log review stays a standing ops habit.)*
