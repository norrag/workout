...
| **3. Engine + MCP** | Measured-FFM pathway in `planMacrocycle` (with N21's correction), RMR context on cut/bulk macros, MCP read tool (`get_body_composition`) reading the same view, coaching-guardrail copy for LSC/comparability. *Built 2026-07-11 (doc 17 Phase 5c): measured bf% rides the existing `bodyFatPct` profile input (§3.1 — no engine change; the 5b apply already writes the profile) with a new provenance column (`profiles.body_fat_source`) so the profile screen shows the measurement instead of the estimate bands while connected (override/disconnect reverts — 09 entry); Cunningham-only RMR context on cut/hypertrophy macro Overviews; `get_body_composition` over `v_body_comp_history` with the §6 guardrails shipped as data. Passing measured FFM directly remains the noted later refinement* | Engine changes ship with golden tests per hard rule 3 |
