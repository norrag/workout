...
| `edit_mesocycle` | in-place structural edits to a `planned`/`active` meso's planner board: **`add_day`** (lay down a whole training day — label/weekday + its muscle-group blocks with exercises and starting sets — in one call, so an empty/placeholder meso builds up to a complete multi-day plan), **`remove_day`**, plus `add_exercise` / `remove_exercise` / `swap_exercise` / `reorder_day` / `set_baseline_sets`. Plus the doc-21 **effort levers**: **`set_exercise_rir`** (slot + a flat RIR, an RIR on named working weeks, an explicit per-week schedule, or `clear`) assigns a target RIR to ONE exercise on ONE day and the engine reprices the load to meet it — **absolute** (it overrides the week's ramp where set, deload included, and the ramp reasserts the moment it's cleared), with a `reason` (A7) surfaced wherever it reads; **`set_exercise_sets`** is the same shape for a working-set cap (distinct from `set_baseline_sets`, which seeds week 1) — recorded and read back, but **the engine does not clamp its set count to it yet** (doc 21 Phase 4), which the tool says on every such write. A week already trained can't be reassigned by name, an assignment below the week's default is warned rather than applied silently, and no progression is earned while a slot runs easier than its week. Returns the fresh plan (new day/slot ids) so a chain of edits needs no re-read. The engine still owns every prescribed number |
