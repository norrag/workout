# AGENTS.md — Operating manual for the notes area

This is **the coding agents' working notebook** for the running stream of field
notes the owner captures while using WORKOUT — bugs, questions, ideas, UX nits, and "go
...
**Coding agents own this structure.** The owner does not browse or maintain
these files — they hand coding agents raw notes periodically and interface with
the backlog *through the coding agent*. So optimize every file here for
**coding agents'** consumption and for surviving across sessions, not for human
readability. Rename, split, merge,
...
> priority defect. The value a coding agent adds is **assessment**: dedup, relate,
...
| File | Role |
|------|------|
| `AGENTS.md` (this) | how this area operates — the standing instructions |
...
get written into the numbered specs (`docs/0x`–`1x`) per the root `AGENTS.md`.
