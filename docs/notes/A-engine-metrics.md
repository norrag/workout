...
| ID | From | Task | Type |
|----|------|------|------|
| T-A1 | S1/PH39 | Reconcile the two e1RM systems (engine anchor vs raw-Epley view). **Done (PR #103, 2026-07-02):** engine e1RM everywhere — `v_exercise_overview`/`v_meso_summary` `best_e1rm` read the stored per-set engine estimate (migration applied live); `v_exercise_history.best_set_e1rm` added for set-grain PR comparisons; the stats layer's inline raw Epley + `epleyE1rm` retired. Stats show the **undecayed** value; recency decay stays prescription-only. 30-day half-life **confirmed MCP-tunable** (`e1rm.recency_halflife_days: 30` in the active v17 params row). Answers PH39. | D→F |
| T-A2 | S3 | Decide + document deload handling in stats. **Done (PR #103, 2026-07-02):** `getMesoProgressScores` excludes deload-microcycle sessions from the first→last trend; total-volume + PR stats keep deloads; denoted in MCP metric definitions (`get_mesocycle_summary`, `get_training_overview`). | D→B |
...
| T-A5 | S7 | Implement graded MEV→MAV→MRV volume ramp + MRV-stop auto-deload, or amend doc 10 to the ±1 model. **Deferred 2026-07-02 (Batch 4):** keep the ±1 model for now; **do not amend doc 10** — the graded ramp stays a documented future option. Owner idea to revisit: expose training style (±1 "German-style" vs. graded ramp) as a **setting / macrocycle-type selection**. Kept in orbit. | D→F |
| T-A6 | PR22/PR23 | ~~Seed a new meso from the recency anchor / rep-based high-water-mark, not just the top-weight PR.~~ **Done (gated, pending activation):** `seed_from_anchor` (§S1) makes `seedMeso` mirror the swap-in `seed_anchor` branch — seeds week 1 from the recency anchor for the window's low rep at the start RIR; `v_exercise_prs` (§S2) now returns a coherent best-e1RM set as the fallback peak. Shipped in engine_params **v11 (inactive)**; activate after a replay diff. See `docs/reviews/2026-06-23-standalone-prescription-investigation.md`. **Follow-on (2026-06-25): retire the prior-peak × back-off fallback entirely → `T-I5`.** **Closed 2026-07-02 (Batch 4):** owner confirmed WS-I resolved this ("Yes, I believe this is addressed and T-A6 can be closed") — swept to `archive.md`. | F |
