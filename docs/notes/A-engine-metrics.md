...
| ID | From | Task | Type |
|----|------|------|------|
| T-A1 | S1/PH39 | Reconcile the two e1RM systems (engine anchor vs raw-Epley view). **Decided 2026-07-02 (Batch 4) → ready:** standardize on the **engine e1RM formula** everywhere (retire raw-Epley `v_exercise_overview.best_e1rm`); **stats screens show the *undecayed / best-ever* value**, **recency decay is reserved for prescriptions only** (stats aren't meant to track strength-over-time via decay). Keep the **30-day half-life**; confirm it's MCP-tunable. Answers PH39. | D→F |
| T-A2 | S3 | Decide + document deload handling in stats. **Decided 2026-07-02 (Batch 4) → ready:** **exclude deloads from strength-progress scoring** (`getMesoProgressScores`); keep them in total-volume + PR stats; denote deloads where relevant. | D→B |
