...
|----|-----------|-------------|----------------|
| **A** | Engine & metrics Q&A | [`A-engine-metrics.md`](./A-engine-metrics.md) | How e1RM, strength gain, progression, misses, baselining, set-planning, deloads work — answered from code + spec. Open: **N71** (the stats e1RM stamp ignores prescribed RIR while the anchor honors it — now doc 21 Phase 1: per-set `rir_reported` capture + one `rir_reported ?? target_rir` resolution, absorbing N38). |
...
| **E** | Logging & feedback UX | _tbd_ | Feedback slider resolution, note icon, back-button clutter, set-type removal, over/under marker, soreness capture. Shipped: replace-sheet filters + confirm step (N48/N49, 2026-07-12); **background set-logging queue** (N68) + thumb-only sliders (N69), PR #215. Open: offline READS (T-N68a, needs an owner call). |
...
| **M** | In-app help & education | [`../22-user-manual.md`](../22-user-manual.md) | Batch 7: shared InfoDot primitive + glossary source for the app's jargon (RIR, e1RM, MEV/MRV, deload…) placed across dense surfaces (N25, _shipped_ — the **term-level** layer). Open: **N74** — the **mechanism-level** layer above it: a full User Guide + a dedicated AI/MCP Manual under `/more/connector`, then a link-placement pass. Plan/IA/phases in doc 22; Phase 0's ground-truth audit gates all prose. |
...
| **P** | Prescribed progression | [`../16-prescribed-progression.md`](../16-prescribed-progression.md) | Earned-step overload + macro-rate pacing (N35, engine_params v20). Phases 1–4 shipped (PRs #158–#161); Phase R activation prepared 2026-07-09. Open deferred spine (doc 16 §11): envelope loop (N36), `rate_source:"plan"` pacer branch (N37, needs N21), required honest-RIR confirmation (N38), per-exercise progression-off override (N39). Shipped: prescription-detail audit surfacing (N44/N45, 2026-07-12). Also holds **N70** (exercise-level RIR — decided A1–A8, build spec [`../21-exercise-level-rir.md`](../21-exercise-level-rir.md), 6 phases, `ready`; the override direction is closed, [`../reviews/2026-07-31-coach-override-prescriptions.md`](../reviews/2026-07-31-coach-override-prescriptions.md)) and **N72** (bounded substitution + the `LOOKBACK_WEEKS = 2` cliff, deferred). N38 is absorbed by doc 21 Phase 1; N39 is redundant under an assignment. |
| **V** | Versioning & releases | [`../23-versioning-releases.md`](../23-versioning-releases.md) | Discrete versioned releases from a fresh v1.0.0 (N80): the repo-side release registry + enforced version identity, `profiles.last_seen_version` and the What's New gate, version history on More, deep links (shared with doc 22's section-ID validator), and the release process — release PR, CI gates, and the announce-then-activate coupling for `engine_params`. Plan written + revised through owner review round 1 (2026-08-06); all §12 decisions closed, `ready` — next is Phase 0's design pass (hard rule 8). **1.0.0 = the framework, 1.1.0 = the manuals**, so this workstream interleaves with **M**/doc 22. |
