...
|----|-----------|-------------|----------------|
| **A** | Engine & metrics Q&A | [`A-engine-metrics.md`](./A-engine-metrics.md) | How e1RM, strength gain, progression, misses, baselining, set-planning, deloads work — answered from code + spec. Open: **N68** (the stats e1RM stamp ignores prescribed RIR while the anchor honors it — now doc 21 Phase 1: per-set `rir_reported` capture + one `rir_reported ?? target_rir` resolution, absorbing N38). |
...
| **P** | Prescribed progression | [`../16-prescribed-progression.md`](../16-prescribed-progression.md) | Earned-step overload + macro-rate pacing (N35, engine_params v20). Phases 1–4 shipped (PRs #158–#161); Phase R activation prepared 2026-07-09. Open deferred spine (doc 16 §11): envelope loop (N36), `rate_source:"plan"` pacer branch (N37, needs N21), required honest-RIR confirmation (N38), per-exercise progression-off override (N39). Shipped: prescription-detail audit surfacing (N44/N45, 2026-07-12). Also holds **N67** (exercise-level RIR — decided A1–A8, build spec [`../21-exercise-level-rir.md`](../21-exercise-level-rir.md), 6 phases, `ready`; the override direction is closed, [`../reviews/2026-07-31-coach-override-prescriptions.md`](../reviews/2026-07-31-coach-override-prescriptions.md)) and **N69** (bounded substitution + the `LOOKBACK_WEEKS = 2` cliff, deferred). N38 is absorbed by doc 21 Phase 1; N39 is redundant under an assignment. |
