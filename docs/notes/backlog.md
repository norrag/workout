...
| N21 | "Realistic" macro-target **engine correction** (audit found: strength target ignores age/sex; hypertrophy model flips discontinuously on profile completeness; cut caps can collapse the range). The interim **hide merged (PR #140)** — cards removed, `planMacrocycle`/`target_*` columns/timeline deps intact, so re-enabling is a pure view change once the engine is fixed. **New consumer (2026-07-08):** N35's macro-rate pacer will read the corrected per-user strength rate (`rate_source: "plan"`), so the strength-path fix (age/sex-aware, currently bucket-only) should expose a per-user monthly rate — see the N35 follow-up §3.4 | Q→D | **HIGH** | C | **NEXT TARGET (2026-07-09)** — primed: [`docs/reviews/2026-07-09-n21-strength-rate-priming.md`](../reviews/2026-07-09-n21-strength-rate-priming.md) scopes the strength-path fix (apply age/sex modifiers currently hypertrophy-only; **strength `sexFactor ≈ 1.0`, NOT the hypertrophy 0.7** — relative 1RM gains are sex-equal; apply `ageMultiplier` w/ a possibly higher strength floor), the hypertrophy model-flip continuity fix, the cut-range collapse guard, and the exposed per-user `perMonthRate` that `rate_source:"plan"` (N37) reads. Elevated MED→HIGH: it blocks N37 + the N36 envelope loop. Build = new engine version (v21 inactive) + replay + activate; re-enable the hidden target cards (PR #140 made it a pure view change). **Architecture record (2026-07-10):** [`docs/reviews/2026-07-10-macrocycle-goals-architecture.md`](../reviews/2026-07-10-macrocycle-goals-architecture.md) answers the owner's four macro-goal questions end-to-end (set → use → measure → carry) and fixes the surrounding decisions: contract-vs-estimate snapshot semantics (persist the `MacroProfile` inputs beside `target_*` at create/edit — N21 slice), plan-rate stays a band + doc-14 derived input (N37 shape), per-user derived `band_position` fold (N36 residence), macro close + retrospective and a bodyweight series filed as N40/N41; also folds in the `key_lifts.n` display drift (top-3 vs param 5, `stats.ts:67-78`) and the `age`→`birthdate` hygiene fix |
...
| N21 | "Realistic" macro-target **engine correction** (audit found: strength target ignores age/sex; hypertrophy model flips discontinuously on profile completeness; cut caps can collapse the range). The interim **hide merged (PR #140)** — cards removed, `planMacrocycle`/`target_*` columns/timeline deps intact, so re-enabling is a pure view change once the engine is fixed. **New consumer (2026-07-08):** N35's macro-rate pacer will read the corrected per-user strength rate (`rate_source: "plan"`), so the strength-path fix (age/sex-aware, currently bucket-only) should expose a per-user monthly rate — see the N35 follow-up §3.4 | Q→D | **HIGH** | C | **NEXT TARGET (2026-07-09)** — primed: [`docs/reviews/2026-07-09-n21-strength-rate-priming.md`](../reviews/2026-07-09-n21-strength-rate-priming.md) scopes the strength-path fix (apply age/sex modifiers currently hypertrophy-only; **strength `sexFactor ≈ 1.0`, NOT the hypertrophy 0.7** — relative 1RM gains are sex-equal; apply `ageMultiplier` w/ a possibly higher strength floor), the hypertrophy model-flip continuity fix, the cut-range collapse guard, and the exposed per-user `perMonthRate` that `rate_source:"plan"` (N37) reads. Elevated MED→HIGH: it blocks N37 + the N36 envelope loop. Build = new engine version (v21 inactive) + replay + activate; re-enable the hidden target cards (PR #140 made it a pure view change) |
| N34 | BodySpec DEXA integration (optional, per-user OAuth connect → scan import → profile enrichment → macro outcome verdicts). Assessment done: [`docs/15-bodyspec-dexa-integration.md`](../15-bodyspec-dexa-integration.md) — API viable (user-tier OAuth2/PKCE, pull-only), schema = `body_scans` time series + `external_connections` + `v_body_comp_history`, engine touch = measured FFMI into `planMacrocycle` only (never set-level), LSC honesty guardrails. Relates to N21 (macro-target correction should assume scan data may exist). Amends doc 01 out-of-scope if adopted. ~~Phase 0 is human-only~~ **Phase 0 resolved (2026-07-10, doc 15 §8):** owner confirmed private single-user deployment; live probe found the Keycloak realm allows **anonymous dynamic client registration** and grants `offline_access` — no BodySpec contact/approval needed; client self-registers at Phase-1 build time (one residual 5-min verification at first login: doc 15 §8.3). | F | MED | — | needs-input — assessment shipped (PR #150), readiness addendum doc 15 §8 (2026-07-10); build is unblocked; owner: adopt & phase? (doc §5) |
...
| N40 | **Macrocycle close + retrospective** ([architecture doc](../reviews/2026-07-10-macrocycle-goals-architecture.md) §3.4) — today nothing happens at macro end: `macrocycles.status` allows `completed`/`archived` but **no code path ever writes them** (macros stay `active` forever); the Overview is a live to-date rollup; no verdict concept exists outside unbuilt doc 15 §3.2. Design: (1) explicit owner-initiated close (⋮ menu / MCP write), nudged — not forced — when `target_end_date` passes; (2) a **derive-on-read retrospective** on the completed Overview + `get_macrocycle_summary`: strength verdict = key-lift measured e1RM gain vs the **stored contract snapshot** (`target_*`/`rate_*`, never the live recompute — §1.4 contract-vs-estimate semantics), demand-side summary from the `get_progression_history` fold (earn/miss mix, pacer pressure), adherence/volume tiles; mass rows say **"not measured"** until body data exists (N41/N34) — never proxy-graded; (3) hard-rule-8 mockup + 09-changelog entry required before build (no figure exists for a completed-macro Overview). Depends on the N21 slice persisting the `MacroProfile` snapshot beside the target columns. Relates: N34 (DEXA verdicts slot into the same card), N41 | D | MED | C | needs-input — design recorded (architecture doc §3.4 + §6 decisions 4–5); owner: close semantics + retrospective v1 scope |
| N41 | **Bodyweight time series** ([architecture doc](../reviews/2026-07-10-macrocycle-goals-architecture.md) §4.3a) — `profiles.bodyweight` is a scalar, so every mass-denominated macro goal (hypertrophy lb / cut lb / maintain) is **ungradable in-app**; this is the single biggest measurement gap in the goal loop. Minimal shape: `bodyweight_log (user_id, measured_on, weight)` owner-RLS + append from the profile edit + optional quick-entry; N34's `body_scans` rows join as high-quality points. Unlocks: N40's mass verdicts, cut leanness-band freshness, "bodyweight as of" honesty labels. The cheap half of N34's value, worth having even if DEXA adoption stalls. Never auto-writes `profiles.bodyweight` (proposal flow per doc 15 §2.3) | D | MED | C | needs-input — design recorded (architecture doc §4.3a + §6 decision 7); owner: adopt standalone, fold into N34, or decline |
| N42 | Est-strength rework: aggregated macro strength dropped the moment a new meso started (owner). Root cause: pure first→last two-point delta let a fresh block's light RIR-ramp opener define the endpoint and crater continuing lifts; and the Overview tile (top-3 key-lift mean) was a *different* aggregation from the Performance muscle rollup (archived N16 only partly closed it) | B→F | HIGH | A | **done (PR #157).** New pure `engine/strength.ts` `strengthTrend` (recent-best vs baseline-best, symmetric rolling windows; `engine_params.strength` optional → replay-safe, falls back to `DEFAULT_STRENGTH`); `foldProgressScores` uses it; headline = volume-weighted mean of the muscle rollup — one definition for the tile + tab. Confidence persisted (`logged_sets.e1rm_confidence`, migration `20260708000001` + backfill/restamp). Clarity: glossary rewrites + InfoDots + RIR in the history flip. Session value stays session-avg (N2). Supersedes archived N16, extends N9. *(Filed as N36 in the PR session; renumbered twice at merge — N36–N39 were taken by the doc-16 deferred spine, N40/N41 by the 2026-07-10 architecture doc.)* Spec: 10 §1/§6/§8. |
