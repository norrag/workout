...
| N14 | Macro muscle-group rollup: bogus "starting e1RM of 7" (hack squat/quads, Jul-24→Dec-25 bulk) — `foldProgressScores` lets one unrepresentative first session define the %-change denominator; the offending session is also invisible in history (120-set cap → N30). Fixed: sessions >3× from the window median are dropped before the first→last fold (`dropE1rmOutliers`) | B | HIGH | C | **done (PR #140)** |
| N15 | Macro muscle groups drill all the way to a macro/meso-scoped exercise history; default e1RM view, tap to sets/reps (inverts the PH32 default for this entry point) | F | MED | C | ready — unblocked (N14/N16 shipped PR #140); ride with N30 |
| N16 | "EST. STRENGTH · KEY LIFTS" tile (-36.3% on the Dec-25 cut) contradicts the Performance tab — bespoke `buildMacroStats` fold includes deloads, means only the 3 most-logged lifts, no qualification. Fixed: tile derives from the qualified pipeline via pure `keyLiftStrengthPct` (top-3 qualifying by frequency) — one definition with the tab; MCP inherits | B | HIGH | C | **done (PR #140)** |
| N17 | Planner: no way to edit # of sets per exercise — `initial_sets` was plumbed end-to-end (model→board→save→engine seed); shipped the missing −/＋ START SETS stepper on filled rows (staged edit + live-draft `updateFillSetsAction`, clamp 1–20) | F | HIGH | D | **done (PR #140)** |
| N18 | RIR ramp editable at create + per-week independent RIR. **Part A done (PR #140):** FinalizeSheet's ramp summary line is a collapsed disclosure → START/END RIR + deload controls, threaded through `finalizeSchema`/`finalizeDraftMeso` (standard defaults, no badgering). **Part B open** (per-week — needs `rir_schedule` override + `rirRamp` hook + doc-14 fingerprint scoping), medium-large, own slice | F | MED | D | A done (PR #140) / B triaged |
| N20 | Enter-share-code in the new-cycle tray too — `RedeemForm` mounted in `NewCycleButton`'s sheet under OR ADD FROM A CODE, mirroring the template tray (redeem is kind-agnostic) | UX | LOW | D | **done (PR #140)** |
| N21 | "Realistic" macro targets: audited (strength target ignores age/sex; hypertrophy model flips discontinuously on profile completeness; cut caps can collapse the range) — **hide shipped (PR #140):** REALISTIC TARGET card (macro overview) + YOUR TARGET range/rate/rationale (create flow) removed; `planMacrocycle` + `target_*` columns + `plan.mesoCount`/`phases` kept, so re-enabling is a pure view change. **Open: correcting the target engine itself** | Q→D | MED | C | hide done (PR #140) — engine fix needs-decision |
