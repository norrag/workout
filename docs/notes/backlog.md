...
| N91 | **ChatGPT holds a frozen tool catalog, so long-connected users are silently missing tools** (owner, 2026-09-12). *"ChatGPT appears to keep a frozen snapshot of an MCP connector's tool catalog until the user manually refreshes it… I do not want to show a generic 'there may be new tools' message to everybody."* The refresh path (web → Settings → Plugins → Workout → Refresh, control at the bottom of the tool list) is ChatGPT's UI and unchangeable; the only thing WORKOUT can improve is knowing **who** is behind. | F | **HIGH** | K | **done (PR #261)** — per-connection catalog tracking. `mcp_client_catalog` keyed `(user_id, client_id)` (so ChatGPT and Claude go stale independently); a **sha256 over the tool definitions as served** plus the instructions plus a generation counter, so a tool change moves it without anyone remembering a bump; recorded on `tools/list` **after** the admin filter and compared on `tools/call`, both wired to the SDK's request handlers rather than to any tool. **Two catalogs** (admin/standard) are fingerprinted separately — a single-catalog design would mark every non-admin permanently stale, and a test asserts against that. Notice = one extra `content` block + `_meta`, once an hour per connection, `structuredContent` untouched. **Unknown = stale**, deliberately: a one-time migration for every pre-tracking connection, cleared by one Refresh. `MCP_SERVER_VERSION` also unpinned from `0.1.0` (now the release version) and `version.ts` records why it is *not* the staleness signal. Docs: `05` §Tool-catalog freshness, Guide ch. 18 §keeping-the-tools-current, `22a` `C-ai-13`–`C-ai-17`. **Migration `20260912000001` applied to hosted 2026-09-12** (RLS + policy + indexes verified, advisors clean); the post-deploy verification steps are runbook items ②–④. |
...
### Batch 41 — stale connector tool catalogs (2026-09-12, session task)

> "ChatGPT appears to keep a frozen snapshot of an MCP connector's tool catalog
> until the user manually refreshes it. Some WORKOUT users connected months ago
> and are therefore likely missing tools that have since been added.
>
> The current manual refresh path is: Open ChatGPT on the web → Settings →
> Plugins → Workout → Refresh. The Refresh control is at the bottom of the tool
> list. This is currently awkward for mobile users, but we cannot change
> ChatGPT's UI. What we can do is make WORKOUT accurately detect when a
> particular user/client connection has not fetched the current tool catalog and
> tell only those users that they need to refresh.
>
> I do not want to show a generic 'there may be new tools' message to
> everybody. […] Please design this so we are not dependent on developers
> remembering several unrelated manual steps every time a tool changes. If the
> actual registered/served MCP tool definitions can be fingerprinted reliably,
> consider whether that should be part of the solution rather than depending
> entirely on a manually incremented integer. […] Do not intentionally expire
> OAuth sessions, revoke grants, change the MCP URL, or break old tool calls
> merely to force rediscovery. Authentication/reconnection and tool-catalog
> freshness should remain separate concepts. […] Also clean up the MCP server
> versioning. MCP_SERVER_VERSION is currently stuck at 0.1.0."
>
> *[→ **N91**. Built as designed: the fingerprint is the mechanism and the
> generation counter is a lever on top of it, not the other way round. The one
> design call the owner left open — what to do about connections that predate
> tracking — was resolved as "unknown is stale", which is also what makes the
> owner's own already-current connection exercise the full test loop on deploy.]*
