...
| N82 | **Day-view focus pass** (owner, 2026-08-14, session task). *"The Workout day view … has gained a number of visual elements that are beginning to create clutter and distraction … retain functionality, ease and speed of use, and quick access to useful tools while reducing visual clutter and focusing the user on the elements that matter most and are used most frequently."* Assessed against the rendered screen, not the spec: the hot path is the **set grid**, and the support furniture had accumulated at the same visual weight — worst as the per-exercise **four-button icon row**, which on a six-exercise day draws **24 bordered boxes** down the right edge, one of them (`note`) an exact duplicate of a `…` row. Five changes, all presentational or menu-structural — no action removed, no write path moved, no engine input touched: (1) the icon row collapses **4 → 1**, the surviving `…` gaining a real 44px target; (2) the **exercise name becomes the prescription strip's disclosure** (name + chevron — the header's own navigator idiom, reused rather than invented); (3) the two note strips **merge into one** under a lighter rule, so the program's voice (heavy rule) and the lifter's stop looking identical; (4) `History ›` **returns to the `…` menu** beside `View exercise ›` (the 2026-06-26 entry folded them together *because* the card had a history control — that premise is gone), and the menu's rows are **grouped** by a stronger rule, **order untouched**; (5) `TARGET n RIR` states itself in **ink, not accent** — hard rule 7 reserves orange for position/selection, so on the day view it now means "where you are" and nothing else. Deliberately untouched: the set grid, per-exercise next-row emphasis (restricting it would break supersetting), the repeated grid header, equipment on the name row. Design record: [09-changelog 2026-08-14](../09-design-changelog.md); inventory + manual corrected in the same PR | F | MED | — | **done (PR #TBD)** — staged behind `releaseActive("1.1.0")` per doc 23 §9.2; release note `day-view-focus-pass` in `unreleased.ts` |
