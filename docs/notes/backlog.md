...
| R11 | Reconcile's unbounded `engine_decisions` fetch truncates at the PostgREST row cap → open rows misclassified decision-less and re-seeded off the prior-meso peak | B | MED | G | **done (PR #119)** |
| R12 | Custom bodyweight exercises stored `load_type='external'` forever (wrong e1RM/effective-load math; app + MCP) + MCP create/search equipment as bare string + dup muscle-group crash leaves orphan exercise | B | MED | G | **done (PR #119)** — no backfill needed (zero custom/bare-bodyweight rows on hosted, verified) |
