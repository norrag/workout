...
| N40 | **Macrocycle close + retrospective** ([architecture doc](../reviews/2026-07-10-macrocycle-goals-architecture.md) §3.4) — today nothing happens at macro end: `macrocycles.status` allows `completed`/`archived` but **no code path ever writes them** (macros stay `active` forever); the Overview is a live to-date rollup. **Closeout semantics decided (owner, 2026-07-10):** close **naturally** when every positioned meso reaches a terminal state, or by an explicit **"End macrocycle"** that irrevocably ends open work — mirroring the `endWorkout`/`endMesocycle` family one level up (logged → `endMesocycle`, never-started → `abandoned`; history never touched); **not** end-date-driven (a user may run past the plan). Retrospective (derive-on-read, on the completed Overview + `get_macrocycle_summary`): strength verdict = the **PR #157 est-strength rollup** (headline + per-muscle) vs the **stored contract** (`target_*`/`rate_*`, never the live recompute), demand-side summary via the `aggregateProgressionEvents` fold (earn/miss mix, pacer pressure), adherence/volume; mass rows say **"not measured"** until body data exists (N41/N34) — never proxy-graded. Completed macros freeze (goals edit refused). Hard-rule-8 mockup + 09 entry first (no figure exists). Build spec: [doc 17 §4 / Phase 3](../17-macrocycle-goals.md). Relates: N34 (5b verdicts slot into the same card), N41 | D→F | MED | C | **ready — doc 17 Phase 3** (soft-depends on Phase 1's `plan_inputs`; grading works off existing `target_*`) |
| N41 | **Bodyweight time series** ([architecture doc](../reviews/2026-07-10-macrocycle-goals-architecture.md) §4.3a) — `profiles.bodyweight` is a scalar, so every mass-denominated macro goal (hypertrophy lb / cut lb / maintain) is **ungradable in-app**; this is the single biggest measurement gap in the goal loop. Minimal shape: `bodyweight_log (user_id, measured_on, weight)` owner-RLS + append from the profile edit + optional quick-entry; N34's `body_scans` rows join as high-quality points. Unlocks: N40's mass verdicts, cut leanness-band freshness, "bodyweight as of" honesty labels. The cheap half of N34's value, worth having even if DEXA adoption stalls. Never auto-writes `profiles.bodyweight` (proposal flow per doc 15 §2.3). Build spec: [doc 17 §5 / Phase 4](../17-macrocycle-goals.md) — `bodyweight_log` w/ `source ('manual','profile','dexa')`, profile-edit append, quick-entry affordance (09 entry first), ±14-day bracketing tolerance for the N40 mass verdict, plus the **create-flow priming line** (prior block's measured strength rate beside the model band, display-only — never blended) | D→F | MED | C | **ready — doc 17 Phase 4** (wants the N40 retrospective for its verdict rows) |
