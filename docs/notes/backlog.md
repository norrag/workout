...
> #226 and is still dark today). Check both before calling something shipped.
...
| N87 | **CI runs `engine_params` v18. Production runs v27.** (found 2026-08-14 while diagnosing N84.) A fresh local stack — which is exactly what the e2e, RLS and integration jobs build on — activates whatever the migrations activate, and **the last migration that flips `is_active` is `20260702000004`, which sets version 18**. Every version since is inserted `is_active = false` and activated by a human through the `engine_params` MCP tools, which write straight to hosted and leave nothing in `supabase/migrations/`. **Four versions have no migration in the repo at all** — v22, v24, v25 and **v27, the live one** — so the repository cannot even reconstruct production's engine. The gap is nine versions and it is not cosmetic: v19 hold-week reprice-down, **v20 earned-step progression**, v21 the macro-target correction, v22/v24 `rate_source:"plan"`, v23 the two-component strength model, v25 the envelope loop, v26 the measuring band, v27 the deload/measuring re-cut — **none of it is exercised by any test that reads the database.** Unit and golden tests pass explicit params fixtures and are unaffected; everything DB-backed is affected. It is the probable cause of N84 (b): a test named *"earned prescription"* asserting v20+ behavior against a database where earned-step has never been active. **And #222's drift guard cannot see this** — it compares migration *files* to hosted migration *rows*, and an `engine_params` activation is neither, so this is the same class of drift one level up, in the blind spot of the fix for it. **Options, owner's call:** (1) a migration per activation from here on, so the repo carries the chain and CI runs production's engine — the honest fix, and it makes hard rule 3's *"all tunables come from `engine_params`"* checkable; (2) extend `db:check` to compare the hosted **active version** against a repo-declared expected version — cheap, catches drift without back-filling history; (3) have the e2e job activate the newest migration-defined version (v26) — narrows the gap to one version but still not production. **Recommend (2) now and (1) going forward**; back-filling v22/v24/v25/v27 as migrations is a separate decision because their param blobs only exist on hosted. | B | **HIGH** | K | **open — needs an owner decision between the three options.** Evidence: `grep 'set is_active = true where version' supabase/migrations/` returns exactly one hit (v18); `mcp__workout__get_engine_params` shows v27 active on hosted with `created_at 2026-08-12`. |
...
|----|--------|-------|------|-------|
| [#222](https://github.com/norrag/workout/pull/222) | 2026-08-04 | migration-drift guard, stalled-week recovery, `skipWorkout`, week-boundary gate | **N85** | **open — stale.** Nothing in it is on `main`. Highest-value open PR; needs a rebase and re-review. |
