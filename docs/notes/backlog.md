...
| PH34 | Meso-stats "planned sets" review — what counts as "planned"? completed + remaining prescribed | Q→B | MED | C | **done (PR #84)** — owner ruled "autoregulated projection" (2026-06-30). Built a pure `projectWeekSets` (`queries/volume-projection.ts`): for unmaterialized future weeks, carry the last materialized week's autoregulated set count forward, deload-scaled (mirrors the engine's set carry-forward under neutral feedback), seeded by the planner baseline only when a group never materialized. Wired into both shared surfaces — `buildVolumeMatrix` (stats UI) and `get_muscle_group_volume` (MCP, new `projected` status) — ending the prior baseline-vs-`null` divergence. **Caveat (relayed):** the engine has no forward MEV→MAV→MRV ramp (T-A5 unbuilt), so the projection is flat across accumulation weeks (honest, not climbing); a climbing projection is a separate engine feature. No SQL migration (pure TS). |
