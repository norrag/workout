...
| T-I4 | PR26 | Delete legacy increment block + retire legacy-only params (new engine_params version, migrate old rows, update tests) | F | **ready / unblocked (2026-06-26): v16 active ⇒ the legacy path is now dead in production (rep-window + bodyweight + cold-start cover every live case).** Its own focused PR: re-point the test harness off the legacy `baseInputs` default (7 files, ~38 assertions, `prescribe.test.ts` is all-legacy), strip increment/regression, decide pre-rep-window replay/provenance, retire legacy-only params. Deliberately NOT bundled with the UI PR. |
