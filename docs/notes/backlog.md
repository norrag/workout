...
| R1 | Share redemption is a cross-user copy primitive: `shares_grantee_accept` lets a grantee rewrite `object_id`; service-role copy never verifies owner | B | HIGH | K | **done (PR #95):** policy dropped (migration applied + probe-verified live), `acceptShareCode` asserts owner ownership on every copy path, shares RLS + mocked-service tests added |
| R2 | Guardrails dead: migrations don't apply to a clean DB + RLS test suite stale (dropped `units` col, params v5 assertion) → CI red since ~06-20, checks not required, PRs merge over it | B | HIGH | K | **advanced (PR #95):** both stale assertions repaired (`units`→`display_name`; params version pin → `≥ 10` + single-active-row). Remaining: clean-DB migration ordering (`is_admin()` before `profiles`), commit `rls_auto_enable()`, make CI checks required |
...
| R8 | Engine: joint-pain 3/3 still ADDS a set — doc 10's one hard safety gate unenforced on set additions; no −set response to pain 3 exists | B | HIGH | G | **done (PR #95):** `pain_cut_gate` gated param — pain ≥ 2 vetoes set adds, pain 3 cuts a set + substitution note; engine_params **v17 applied + ACTIVATED** after clean replay (zero set diffs) |
