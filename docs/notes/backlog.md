...
| R2 | Guardrails dead: migrations don't apply to a clean DB + RLS test suite stale (dropped `units` col, params v5 assertion) → CI red since ~06-20, checks not required, PRs merge over it | B | HIGH | K | **done (PR #96, stale assertions via PR #95):** chain now applies to a clean DB — `is_admin()` reordered (recorded rule-#2 deviation), hosted `rls_auto_enable()`+`ensure_rls` captured (`20260619000002`), muscle-group seed-order dependency fixed (`20260611000002`); verified on scratch PG16 (51 migrations + seed; stock data counts match hosted). Spawned T-R2 (perf drift). **Owner step remaining (runbook):** make `checks`+`rls-tests` required on `main` after merge |
