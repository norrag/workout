...
| N67 | **Admin MCP tools for the notes area** (owner, 2026-07-30, Batch 28). "Build an admin gated mcp tool to add to and manage the notes section … able to read and understand the paradigm and structure of the notes section, and … add to it in the exact same way that you do" — so notes can be captured and organised from any connector client as they occur, with Claude Code reserved for the reviews, implementation plans and execution that need full codebase access. **Built:** six admin-gated tools (`get_notes_manual`, `get_notes_backlog`, `read_notes_file`, `capture_notes`, `update_note_item`, `append_notes_log`) over a pure `src/lib/notes/` markdown layer (`markdown`/`backlog`/`archive`/`log`/`readme`/`ids`/`types`), with `repo.ts` (GitHub transport + `docs/notes/**.md` path allowlist) and `area.ts` (snapshot → pure transform → one commit) the only I/O. **The area stays in git, not mirrored to Postgres** — the notes files *are* what a Claude Code session reads, so a DB copy would create a second source of truth against this area's own rule that `backlog.md` is the single source of truth for item state; committing through the GitHub API means a note captured on a phone is in `backlog.md` immediately, no reconciliation. The paradigm is carried three ways: `CLAUDE.md` served verbatim, the vocabulary *enforced* in zod + `notes/types.ts` (a status outside the lifecycle or a workstream outside the roster can't reach a row), and every write protocol-complete in one commit (appendix + rows + `log.md` together) so the stale-index failure is structurally unavailable. Scoping files stay Claude-Code-only by design — remote capture files assessed items, not scope. Concurrency: the branch head is the commit parent, so a racing session write is rejected, never clobbered. 34 pure tests against the real `docs/notes/` files (incl. byte-identical row round-trip; found and fixed a real parse gap — N62's cell carries a `|` inside inline code) + 21 tool tests. Relates N61 (same propose-via-MCP pattern for the coaching prompt) | F | — | R | **done (PR pending — branch `claude/admin-mcp-notes-tool-hlx4nu`)** — needs `NOTES_REPO_TOKEN` set before the tools function (manual-operations.md) |
...
### Batch 28 — admin MCP tools for the notes area (2026-07-30, in-session)

> "I would like to build an admin gated mcp tool to add to and manage the notes
> section of this app. The tool should have or be able to read and understand
> the paradigm and structure of the notes section, and be able to understand and
> add to the notes section in the exact same way that you do. This will provide
> a faster and more interactive way of managing and logging notes as they come
> to me without having to rely on Claude code to do this. This way I can manage
> and prepare my notes and changes via another model, and use Claude code to
> then produce more in depth reviews and implementation plans which require full
> codebase access, and executions of those task, while providing an easier and
> more fluid way of intaking and organizing notes in other sessions."
> *[→ N67; new workstream R. Built this session: six admin-gated tools over a
> pure `src/lib/notes/` markdown layer, committing to `docs/notes/**` through
> the GitHub API so the area stays single-source-of-truth in git. Needs the
> owner to mint `NOTES_REPO_TOKEN` before the tools function — the one manual
> step, in `docs/deployment/manual-operations.md`.]*
