...
| N40 | **Macrocycle close + retrospective** ([architecture doc](../reviews/2026-07-10-macrocycle-goals-architecture.md) §3.4) — today nothing happens at macro end: `macrocycles.status` allows `completed`/`archived` but **no code path ever writes them** (macros stay `active` forever); the Overview is a live to-date rollup. **Closeout semantics decided (owner, 2026-07-10):** close **naturally** when every positioned meso reaches a terminal state, or by an explicit **"End macrocycle"** that irrevocably ends open work — mirroring the `endWorkout`/`endMesocycle` family one level up (logged → `endMesocycle`, never-started → `abandoned`; history never touched); **not** end-date-driven (a user may run past the plan). Retrospective (derive-on-read, on the completed Overview + `get_macrocycle_summary`): strength verdict = the **PR #157 est-strength rollup** (headline + per-muscle) vs the **stored contract** (`target_*`/`rate_*`, never the live recompute), demand-side summary via the `aggregateProgressionEvents` fold (earn/miss mix, pacer pressure), adherence/volume; mass rows say **"not measured"** until body data exists (N41/N34) — never proxy-graded. Completed macros freeze (goals edit refused). Hard-rule-8 mockup + 09 entry first (no figure exists). Build spec: [doc 17 §4 / Phase 3](../17-macrocycle-goals.md). Relates: N34 (5b verdicts slot into the same card), N41 | D→F | MED | C | **done (PR #<n>, branch `claude/macrocycle-goals-phase-3-qvav73`)** — doc 17 §4 / Phase 3 shipped: natural close (`maybeCompleteMacroAfterMeso` at the final-week advance + `endMesocycle`; placeholders don't count as open work), explicit `endMacrocycle` (logged → the `endMesocycle` path, never started/placeholders → `abandoned`; header ⋮ + confirm sheet per the 09 entry), terminal-macro freeze (`goalsEditRefusal` + placement/slot guards), `macroRetrospective` pure fold (strength verdict vs the stored contract with the fixed vocabulary, demand aggregate via `aggregateProgressionEvents` + `combineDemandSummaries`, mass "not measured", block-outcome mix) shared by the completed Overview and `get_macrocycle_summary` (parity-tested); no migration (`completed` already in the status vocabulary, derive-on-read per principle 5). Tests +24 (suite 1015) + e2e closeout flow. N41's verdict-row dependency unblocks on merge |
