...
| N41 | **Bodyweight time series** ([architecture doc](../reviews/2026-07-10-macrocycle-goals-architecture.md) §4.3a) — `profiles.bodyweight` is a scalar, so every mass-denominated macro goal (hypertrophy lb / cut lb / maintain) is **ungradable in-app**; this is the single biggest measurement gap in the goal loop. Minimal shape: `bodyweight_log (user_id, measured_on, weight)` owner-RLS + append from the profile edit + optional quick-entry; N34's `body_scans` rows join as high-quality points. Unlocks: N40's mass verdicts, cut leanness-band freshness, "bodyweight as of" honesty labels. The cheap half of N34's value, worth having even if DEXA adoption stalls. Never auto-writes `profiles.bodyweight` (proposal flow per doc 15 §2.3). Build spec: [doc 17 §5 / Phase 4](../17-macrocycle-goals.md) — `bodyweight_log` w/ `source ('manual','profile','dexa')`, profile-edit append, quick-entry affordance (09 entry first), ±14-day bracketing tolerance for the N40 mass verdict, plus the **create-flow priming line** (prior block's measured strength rate beside the model band, display-only — never blended) | D→F | MED | C | **done (PR #<n>, branch `claude/macrocycle-goals-phase-4-flvrc9`)** — doc 17 §5 / Phase 4 shipped: `bodyweight_log` migration (`20260711000001` — owner RLS + tests, unique `(user_id, measured_on, source)`, same-day replace; `v_macro_summary` gains the `first/last_logged_at` span), appends from every profile-bodyweight edit (editor, day-view chip, onboarding — `source:'profile'`) + the More-page quick entry (`'manual'`, backdatable, **never** writes the profile scalar), `bodyDeltaForSpan` ±14-day bracketing feeds the N40 retrospective's `bodyData` seam (Overview + MCP flip "not measured" → graded Δbw off one fold), "as of" freshness labels (More card, create chip, profile editor — one vocabulary), and the fig-2.3 `LAST BLOCK MEASURED` priming line (`getPriorBlockMeasuredRate`, %/mo over a ≥28-day logged span, display-only — model-band half waits for Phase R2). Tests: +12 unit (suite 1027) / +4 RLS / quick-entry e2e + priming negative. N34 5b's DEXA points now have a series to join |
