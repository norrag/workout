...
| N25 | Info/help screens for jargon app-wide: shared `InfoDot` primitive + one `glossary.ts` source (RIR, e1RM, MEV/MRV, deload, ramp…); migrate the 2 ad-hoc feedback-sheet explainers; place incrementally across dense surfaces | F | MED | M | **done (PR #148)** — `glossary.ts` (11 terms + copy-contract test) + anchored-card `InfoDot`; both feedback explainers migrated; wave-1 dots on day-view header, meso calendar/edit sheets, planner finalize + volume readout, stats volume/performance, exercise e1RM cell. Further placement rides along as screens are touched |
