...
| N74 | **User Manual + AI Manual** (owner task, 2026-08-05; **review round 1 2026-08-06, Batch 33**). Two user-facing manuals plus a follow-on link-placement pass: (A) a **User Guide** — key terms, how to use every screen, plain-language explanations of how the load-bearing machinery works (the strength anchor, the Epley/Brzycki e1RM average over effective reps and the role e1RM plays, volume counting, autoregulation) with the *reasoning* behind each; (B) an **AI Manual** under `/more/connector` — setup, concept, capabilities, and worked use cases for macrocycle creation/management, mesocycle creation/management, performance analysis and coaching; (C) afterward, deep links from dense app surfaces into the exact manual section that explains them. Plan + IA + content contracts + phases: [`docs/22-user-manual.md`](../22-user-manual.md). **Phase 0 is a four-part ground-truth audit that gates all prose** — the docs supersede each other in place (09↔08↔06, 19↔18, 21 amends 11's RIR premise, 16↔17) and v20/v23 shipped **inactive**, so spec prose alone would document behavior that isn't live; Batch 32 (PR #226) then changed four documented surfaces the day after the plan was written, which is the standing argument for the claims ledger. **Round 1 (2026-08-06) — all of D1–D5 and O1–O6 accepted** (D3 conditionally: offline only if launch/load times are untouched) **and the plan revised**: three chapters added (7 RIR ramps ↔ training styles — needs a research pass first, 8 exercise-level RIR with both the rehab and the push-harder use cases, 9 deloads incl. the honest evidence position); **admin content dropped from both manuals** and sign-up/auth dropped as pre-completed; two new copy contracts (**positive framing** — describe what the app is, not what it isn't; **plain language** — "Claude or ChatGPT" not "LLM", "connector"/"plug-in" not "MCP"); **navigation re-cut so the *section*, not the chapter, is the addressable unit** — chapter pages are contents pages, every path to a section is ≤1 tap, and a build-failing section-length budget stops the 100-page-scroll failure mode; D3's condition met by **cache-on-read rather than precache** (sw.ts already CacheFirsts `/_next/static/**`) plus three enforced guards (import guard, precache exclusion, lazy search index). **Owner's two questions answered in-doc:** §10 — connector retrieval is **retrieve-then-read over the authored section graph, no vector RAG** (D2 makes chunking an authorship product; index resource + `search_manual` + `get_manual_section`, glossary as the alias layer, embeddings deferred behind a recall trigger), which promoted that phase from optional and moved it ahead of the AI Manual content; §14 — runtime editing via an admin tool is **not recommended** (a GitHub-write token is a new high-value secret and bypasses the content contracts; DB-backed content forfeits CI enforcement, offline, and a single source of truth), keep content in the repo and hold a narrow **errata overlay** in reserve. Extends **N25** (archived — `InfoDot` + glossary is the term-level layer; this is the mechanism-level layer above it, sharing one glossary). Open: **O7** (how far to name real published programs in ch. 7) | F | — | M | **Phase 0 done (PR #228) — the four ground-truth audits landed as [`22b`](../22b-source-map.md) (supersession + live-behavior ledger), [`22c`](../22c-app-inventory.md) (route-by-route inventory + concept/FAQ map) and [`22d`](../22d-connector-inventory.md) (connector). Three corrections to doc 22 itself: §2.2's inactive list is stale (v20/v23 are LIVE; the real inactive behavior is **v26, the measuring band**, which sits under ch. 8), §5 ch. 10's Brzycki rationale is backwards (Brzycki inflates above ~10 eff reps; the operative rule is a cutoff, not a cancelling average), and §6.1's "MRV-stop rule the app measures" is not implemented (a ±1 workload model ships; doc 10 §3's graded ramp + auto-deload never landed, T-A5). Merged `main` after N80 and complied with doc 23 — **the manuals are release 1.1.0** (doc 23 §11.1), so Phase 2 ships the guide routes behind `releaseActive("1.1.0")` and must populate `GUIDE_SECTION_IDS` for release-note `guide` targets; **doc 22 Phases 1–2 are now the critical path to 1.1.0**. **Phase 1 built (PR #231)** — the design pass first (hard rule 8: figs **4.8/4.9/4.10** derived in `09-design-changelog.md`, no mockup exists), then the nine-kind block union + inline vocabulary, the section-ID scheme (`ug/effort-rir#per-exercise`, already doc 23's shared validator), the length budget **calibrated against real copy** (ch. 6 ran 205–309 words / 6–8 blocks, median 229 ⇒ 350/12 stands), the house-styled renderer, and **chapter 6 end to end in six sections** at `/more/guide/effort-rir`, all behind `releaseActive("1.1.0")`. `figure` deferred to Phase 2 with its asset policy (shares a 64-entry image cache with the app icons). [`22a-manual-claims.md`](../22a-manual-claims.md) opened with **22 verified rows**. **The §8.1 glossary contract caught a live defect in its first chapter**: `GLOSSARY.e1rm` said *"closer to failure reads as stronger"*, the mechanic inverted — e1RM rises with effective reps (`reps + rir`), so the set with reps in reserve implies the greater strength (the doc 21 §2 restamp, +4.85% strictly upward, is the same fact in production). Fixed, pinned by a test, staged for 1.1.0, logged as `D-01`. **O-D closed** (Phase-0a corrections folded into doc 22's prose). **Owner review round 2 (2026-08-07) — signed off with four notes**, all folded in and generalized as doc 22 **§8.4a**: prev/next and a labelled `related` list carrying each target's summary were **pulled forward from Phase 2** (a link must say why it is there; an adjacent section must be one tap); a tenth block kind **`legend`** shows the app's actual mark instead of describing it (the `▲`/`■`/`▼` glyphs moved to a shared `src/lib/set-markers.ts` that `DayView.tsx` now reads too, `D-03`); and **no definition may lean on an unexplained abbreviation** — `GLOSSARY.e1rm` became `ESTIMATED ONE-REP MAX (E1RM)` (`D-02`), so §8.1 caught **two** copy defects and one latent one in a single phase. **Phase 2 built (PR #232)** — the reading surface is now a manual. Design pass first (fig **4.11** guide search claimed; fig 4.8 map built). Landed: the **map** with sections inline (doc 22 §9.2's one tap holds), **search** (`/more/guide/search`) as the lexical retrieve-then-read index doc 22 §10 designed — no embeddings, and the design claim is tested against the real chapter including a hand-authored keyword the prose never says; **deep-link entry** (`?from=` re-points the breadcrumb per N27, allowlist-validated, accent ■ on the landed section); the **`Guide` row on More** (O2); **`figure` + its asset policy** — single-colour line art rendered as a CSS mask filled with `currentColor`, because the app's explicit light/dark switch would make a baked `<img>` vanish in one theme, in its own runtime cache ahead of the 64-entry app-chrome bucket it would otherwise evict; the **five §8 contracts as tests** (glossary identity with a pending-terms ledger that may only shrink, honesty incl. every cited `engine_params` path resolving **against the schema**, the claims ledger parsed and resolved, positive framing, plain language); the **three D3 guards**, the precache one reading the **built** service worker in a CI step after `npm run build`; and **`GUIDE_SECTION_IDS`** populated for doc 23 as literal strings (importing the registry into `links.ts` would have broken guard 1 on its first use — the test does the resolving instead). **Two findings:** doc 22 **D3's third promise is wrong** (`22a` **D-04**) — the reader is server-rendered, so a section is HTML + an RSC payload, both `NetworkOnly` under R7, never a `/_next/static/**` asset; **offline manual reading is not delivered**, and doc 22 §4 withdraws it on the owner's own **O1** framing (worth having only because it was free) — **owner call needed only if that reasoning is not accepted**. And **D-05**: the first pass at guard 2 was written against the wrong `@serwist/next` entry point, caught by `tsc`. **Phase 3a built (PR #233)** — the first content group: chapters **1 What WORKOUT is**, **2 Your profile**, **3 The cycle model**, twelve sections at 139–243 words against the 350 budget (corpus median 229 → 211, so ch. 6's mechanism section is still the one brushing the ceiling). No design pass needed — no new block kind, which is the point of having built the surface first. **`day_slot` added to `glossary.ts`**, the first of `22c` §C2's ten, because ch. 3 depends on it and §8.1 forbids a manual-only definition; **the other nine land with the chapters that need them**, so each definition is authored by the pass that verifies the behavior behind it. The pending-terms ledger shrank by three — `macrocycle`/`mesocycle`/`microcycle`, the core vocabulary `22c` finding **C1-a** flagged as defined-but-never-surfaced, now rendered as cards (an in-app `InfoDot` for them is Phase 7a's). The live row was **re-read rather than trusted** (`get_engine_params(25)`: v25 still active, hash-verified, `max_measuring_rir` still absent), and the five ch. 2 parameters were added to `22b` §4.2 under its own rule; 38 new `22a` rows. **Two findings, recorded not fixed** (content phase, §1.2 forbids behavior changes): **`D-06`** — `preferred_equipment` (the `EQUIPMENT ACCESS` toggles) has **no consumer in the app at all**, only the connector's `get_profile`, so toggling `barbell` off changes nothing a user can see — either wire it into the pickers or drop the field; **`D-07`** — the profile's body-fat copy says a blank field falls back to training age, one step short of what v21's `bf_proxy_pct` actually does. **Owner review round 3 (2026-08-09)** returned seven content notes and a navigation reversal, all folded in and generalized as doc 22 **§8.4b** so they bind every later chapter — **ch. 3 was signed off as the composition model** (*"focused and substantive"*). The **map reverses to chapters-only** (fig 4.8 amended one day after Phase 2 built it): listing every section inline was right at one chapter and a ~130-row wall at 21, which is the untraversable-document failure §9 exists to prevent moved onto the orientation screen; the chapter page joins the critical path and takes **chapter-level prev/next**. The rules: orientation before detail · substance per sentence · distill, never describe a description · the reader's words not the build's · never define a thing by what it is not (incl. rhetorically) · weight follows importance · draw what is structural. Two are corrections to earlier readings — 22c §B5.2's "extend, do not restate" meant *take it further*, not *quote-then-gloss*, and §8.4's test passes a rhetorical negation that the rule forbids. Ch. 1 lost its version-history section (ch. 19 owns it per 22c §B5.1a) and its Workout-tab section became an orientation to the Workout page; ch. 2 §1 was re-proportioned; ch. 3 gained a nesting figure with the profile drawn outside the stack as an input. **Phase 3b built (PR #234)** — chapters **4 Planning a mesocycle** and **15 Exercises & templates**, six sections each, 138–228 words (corpus median 225 → 215 over 29 sections), the first group written under **§8.4b** with ch. 3 as the composition model. Ch. 4 draws the planner board's day → muscle-group → slot nesting as a `figure` (rule 7 — the open slot dashed, which is the board's own mark for empty; 09-changelog 2026-08-10) and carries N78's real payoff: the board reopens **mid-block**, edits stage until `SAVE CHANGES`, and everything logged is untouched. Ch. 15 carries N67 (load steps index off the last weight **entered** — 88 lb with a 10 lb step goes to 98 or 78), the three bodyweight load meanings verbatim, and share codes as copy-on-accept. **`load_step` added to `glossary.ts`**, the second of `22c` §C2's ten under 3a's each-term-lands-with-its-chapter rule. **§8.4b rule 1 did real work**: both chapters *name* MEV/MRV, `EST. 1RM`, `BACKED OFF` and the ramp's values without defining any of them, because ch. 12/10/8/6 own them. **Three findings, none fixed** (content phase): **`D-08`** — the create-mesocycle sheet hardcodes `DELOAD AT 4 RIR` while the live `deload.target_rir` is **6**, so the one screen where a user sets the deload up is the one that misstates it (every other surface reads the parameter); **`D-09`** — `22c` §B2.6 tabulated **four** ways to start a block from the page's copy, but `Meso builder` renders disabled with `" (soon)"` — doc 22 §2's failure mode occurring *inside* the audit written to prevent it, corrected in 22c in place; **`D-10`** — N46's missing template-edit path, stated by ch. 15 as the positive rule. **Round 4 (2026-08-11) — three notes on ch. 4, all folded in and generalized as doc 22 §8.4c** (binding on 3c onward), plus one design proposal spun out as **N81**. All three notes are the same failure: a chapter documented its own surface accurately and left the reader worse off than the app would have. (1) **Name a better path even when another chapter owns it** — ch. 4 listed three in-app routes into a block and was silent about planning one through the connector, which is the most capable and usually the easiest route; §1 now gives it a heading and two paragraphs, names what it is better at, and hands off (`create_mesocycle` lands **planned**, `activate_mesocycle` is confirm-gated and told to prefer in-app activation). The typed cross-link is owed once ch. 18 / the AI Manual exist. (2) **Define an advanced term where the reader meets it** — reading position sets *depth*, not whether a term is defined at all; §4 renders the `volume_landmarks` card rather than putting `UNDER MEV` / `OVER MRV` in front of a reader eight chapters before ch. 12 defines them (pending-terms ledger shrank by one). This narrows §8.4b rule 1. (3) **Before writing "here is where you do X", find every surface that does X** — §6 had documented the planner board as the only way to change a running block; the day view's exercise ⋮ menu reorders, swaps, sets effort targets and adds exercises too, and carries `Repeat this change on this day in future weeks` (a reorder propagates with no checkbox at all). **Phase 3c built (PR #235)** — chapter **5 Training a session**, six sections, 206–256 words (corpus median 215 → 218 over 35 sections); the first chapter written under §8.4c and the first content PR since Phase 2 with **no design surface at all** — no new block kind, no asset, and no glossary term owed (`22c` §C2 marks every undefined string on this screen manual-only). It walks the day screen end to end — the header and the progress rule (**skipped sets leave its denominator**, which is what separates *cut it short* from *failed to finish*), the navigator, the set row with **N68 stated as the reader meets it** (a tap is recorded on the phone and sent when it can be, so a dead spot in the gym cannot strand a session), the two menus as intent → control, when each feedback prompt appears, and the completion sheet plus the two early-exit paths that keep everything logged. **Three seams held rather than restated**: ch. 6 owns the RIR box and the above/below-the-ask consequence (linked twice inline), ch. 4 owns the controls that edit the *plan*, and ch. 17 owns the prescription strip's layers — the last of those also a live constraint while **O-A** is unanswered. One forward debt: §5 owes ch. 11 a typed cross-link, which lands with ch. 11 in 3e. **31 new `22a` rows, no new defects.** **Phase 3e built (PR #235, with 3c)** — chapters **11 Why the app asks how it felt** and **12 Volume**, five sections each, 157–227 words (corpus median holds at 215 over 45 sections), taken **ahead of 3d** because 3d is gated on the ch. 7 research pass and these two are what ch. 4 and ch. 5 were already handing off to. Both chapters sit on `22b` §7's spec-vs-code gaps and are written from the code: **the ±1 model is what ships** (doc 10 §3's graded MEV→MAV→MRV ramp and its two-week-at-MRV auto-deload were deferred as T-A5, so ch. 11 describes one set off, one set on, or hold, and no automatic deload), and **MEV/MAV/MRV is a classification library** (ch. 12: the planner board is the one screen that calls a count high or low; the stats report without judging). Three things neither spec states: the set-add branch needs **four** conditions at once (easy workload · strong pump · a growth goal · under `mg_set_ceiling`) with a pain veto over all of them; **`session_dampen_require_both` is `true`** on the live row, so a hard session you performed well in still gets its increase — the hold takes fatigue ≥ 8 **and** performance ≤ 3; and volume's logged counts apply a **hard-set filter** (non-warm-up, ≤ 4 reps in reserve, unreported counting) that lives in `v_meso_week_muscle_sets` rather than in `engine_params`. Pending-terms ledger **shrank by three** (`pump`, `workload`, `fractional_sets`); three remain, each with the chapter that owes it. Four parameters added to `22b` §4.2 from a fresh read of the live row (still v25, hash-verified). **`D-12`** — `22c` §B2.4 listed `TOP SET BY WEEK — KEY LIFTS`, which N10 removed on 2026-07-03; corrected, and its §C2 glossary recommendation flagged for re-siting before **ch. 13** inherits it. One contract-machinery change: §8.2's current-value check now recognises top-level parameter paths (`pain_gate`, `workload_high`, `min_sets`) when the schema resolves them, dotted-path existence unchanged. Ch. 5's forward debt paid — its feedback section links into ch. 11. **Round 5 (2026-08-11) — four notes across 3c and 3e, folded in and generalized as doc 22 §8.4d** (binding on every later chapter): (1) **where the reader enters a value, say what it does and who reads it** — the counterweight to §8.4b rule 1, since a reader lands on the screen chapter or the mechanism chapter with equal probability and both must leave them able to proceed; (2) **an overview section is an answer, not an index**; (3) **never claim a virtue by negation** — praising the app for an absence of friction, a third shape of the failure §8.4 and §8.4b rule 5 already chase, now a denylist test. **Ch. 5 gained a seventh section, `#notes`**, on the owner's point that notes are legible to the connector — *a note to yourself and to your training coach*; verified against `get_exercise_notes`, `get_exercise_history` (`session_note`) and `get_recent_sessions` (`notes`), and the chapter states the seam both ways because nothing in `src/lib/engine/` reads a note. It was **split out rather than appended** — the addition took `#adjusting-as-you-go` ten words over budget, the first time §9.3 forced a structural decision. Ch. 5 §6/§7 now state what each answer moves and link into ch. 11; ch. 11 §1 is a five-row answer covering every field including the two the app records rather than acts on. The denylist caught a fourth sentence the review had not named, and hit `GLOSSARY.e1rm`'s *"so you never have to test one"* — which §8.1 makes the app's copy, so the check now runs over **authored** prose (`authoredProseOf`) while the honesty and hype checks keep reading term bodies. **Phases 3d-r + 3d built** — the **research pass** ([`2026-08-11-rir-ramps-and-training-styles.md`](../reviews/2026-08-11-rir-ramps-and-training-styles.md)) read all seven sources first-hand instead of summarizing doc 10's summary of them, and **that is what caught doc 10's own rationale being wrong**: §4 justifies the RIR ramp with *"hypertrophy gains flatten past ~1–2 RIR"* and `COACHING_GUIDE` repeats it verbatim, but neither cited paper establishes a plateau — Refalo 2023's failure-vs-non-failure effect is small but positive throughout (ES 0.19, CI 0.00–0.37) and the Robinson meta-regression found a *continuing* negative slope with intervals excluding null. Doc 10's conclusion survives (`0 RIR` is a peak-week ceiling); its argument does not, so the manual states the **trade** — a small per-set gain, a steep per-set fatigue cost (bar speed −8% at 3 RIR / −13% at 1 / −25% at failure), and fatigue as what limits weekly sets. Logged as **`D-13`**; doc 10 and `coaching-guide.ts` are theirs to correct. **O7 closed at its conservative end — no third-party program is named**: the literature studies proximity to failure as a *variable*, not as a published program's schedule, so a name would have rested on that program's commercial materials, which the claims ledger cannot verify (review §6 records the one-`detail`-block reversal). Chapters **7 Choosing your ramp**, **8 Exercise-level RIR** and **9 Deloads**, five sections each, 172–255 words (corpus median 215 → 212 over 61 sections), written together because the three levers are one system. Two behaviors no spec states: the doc 21 §6.2 back-off policy is **asymmetric** and the asymmetry *is* the reassurance (easier work leaves the strength trend, `best_e1rm` and the PR view, and **stays** in volume, `weight_pr`, `volume_pr` and `total_volume` — observations, not estimates — with the excluded sets counted as `backed_off_sets` rather than vanishing); and the earn gate **refuses explicitly** with reason `exercise_rir`, which ch. 7 reaches from the other side (a ramp whose easiest week is ≥ 4 RIR yields only `low`-confidence estimates against `progression.min_confidence: moderate`, so it holds the weight by construction). **Ch. 9 is the chapter the specs would have got wrong** — doc 22 §6.1's own MRV-stop rule is not implemented (`22b` §7, T-A5), so the chapter says the app **deloads on a schedule and nothing triggers one**, and its evidence section carries the nuance doc 10 lacks: Coleman 2024's deload group **stopped training for a week**, a different intervention from the light week this app prescribes, so the trial argues against skipping training mid-block and says little about a deload week. Per `22b` §4.1 ①, **the measuring band appears nowhere** — v26 is inactive, so ch. 8 gains it in the release that activates it (**O-B**). `deload` rendered ⇒ pending-terms ledger down to two (`e1rm_confidence` → 3f, `est_strength` → 3g); 48 new `22a` rows; `GUIDE_SECTION_IDS` +15. **Phase 3f built** — chapter **10 How your next weight is chosen**, the headline chapter, six sections, 185–292 words (corpus median holds at 215 over 67 sections), **and its owner review gate is open**. Every chapter so far has been handing off to it. Structured as the argument: a set becomes an estimate · the estimates fold into one anchor · the anchor prices a weight · a clean week earns one step on top. The `22b` §6.1 correction is stated as a **reason** rather than a rule — the two formulas agree over short heavy sets and one runs away upward past ~10 effective reps, so averaging is right inside that band and wrong above it (ch. 6 already carried the cutoff itself). **Earned vs offered** is the split that makes progression explicable: §5 is *did last session earn a step* (eight predicates in the reader's words), §6 is *is now when it gets spent* (the pacer plus three governors), and doc 16's **budget, never quota** closes the chapter. Three facts no chapter had carried: the rep climb **rides the RIR step** (`climb_requires_rir_step`), topping out is judged on the **lowest** performed working set (`climb_on_performed_reps`), and `progression.goal_rate_factor` is **0 for cut and maintain** so those blocks earn no steps at all. **`D-14` fixed here** — `GLOSSARY.e1rm_confidence` closed with a sentence describing the **measuring band** (v26, inactive), so the card claimed a rating tier no user has; ch. 10 is the first chapter to render it, and §8.1 (carry the app's words) collided with **O3** (live behavior only) on that one sentence. Removed, with a code comment carrying the exact text to restore when v26 activates. Third §8.1 catch, first **O3** catch; the card has no `InfoDot` call site, so no reader had seen it. Live row re-read a third time (still v25, `91887f0f…`, hash-verified, `max_measuring_rir` still absent); ten parameters added to `22b` §4.2. 23 new `22a` rows; `e1rm_confidence` rendered ⇒ `est_strength` (3g) is the last pending term; `GUIDE_SECTION_IDS` +6. **Owner review round 6 (2026-08-15) returned notes across ch. 7–10, generalized as doc 22 §8.4e** (six rules), two of which are about what the manual *believes* rather than how it writes. (1) **The effort cluster had drifted cautious**: *"leaving reps in reserve is a fatigue-management tool, not a growth tool… most people probably do not train hard enough."* Ch. 7 §3 now leads with that, adds the practical case for visiting failure (the only way to calibrate what your own 0 feels like), gives each ramp one `Set as` value, and closes on effort and consistency as the drivers; §4's guidance is *do not train easy all the time*, with **bar speed** as the in-set cue; §5 fixes a causality error — a hard ramp does not mechanically cost sets, reporting sessions as too much does. (2) **Parameter names left the prose** — nine `code` runs in layer 1–2 across six chapters, two in the signed-off ch. 2 and ch. 6, all moved into `detail`. A new test fails the build on a parameter name outside a `detail`, and §8.2's current-value check moved from block to **section** scope so the grep chain survived; where the reader now gets a derived relationship (*"3 or fewer reps short of failure"*), the ledger records the derivation (`C-ramp-16`). (3) **`D-20` — ch. 8 answered a question its own specifier got wrong.** The owner asked whether backed-off sets leave the strength anchor; **they do not** — `queries/anchors.ts` filters only on `isMeasuringRir`, which is `true` for every set while v26 is inactive, so there is no 8-RIR cutoff and no 7–5 band. Doc 21 §5 wants it that way. The owner's *outcome* intuition was right; the *mechanism* is the confidence ladder, not exclusion. (4) **Ch. 9 lost a section and gained a control** — *what a deload is* and *when you need one* merged; the *valve* / *shedding* / *performance-debt* framing went; "halved" is now unmistakably about sets; and the chapter documents what the draft had missed outright: a started deload **can** be dropped via `End mesocycle` in the day view (or `Skip remaining sets` per exercise), with attendance untouched because deload days never counted. (5) **Ch. 10 states the real formulas** (Epley `w × (1 + r ÷ 30)`, Brzycki `w × 36 ÷ (37 − r)`) and corrects itself on recency: **age decides which session wins, not what it is worth** — the value is that session's mean at full value, and nothing stored ever fades. One note spun out as **N83** (bar speed as in-app guidance — a glossary card or a coaching-line trigger; hard rule 8, joins N81 in Phase 7a). Reconciled onto `main` after Phases 3g–3i/5/6 (and the unrelated N82 day-view focus pass) landed independently — `D-15` renumbered to **D-20** and the proposal's number renumbered to **N83** to clear collisions with work merged in between. **Phase 3g built** — chapters **13 Reading your stats** and **14 Macrocycle goals**, eleven sections, 148–305 words (corpus median 215 → 209 over 78 sections). **The phase where the audit's own reading of a screen was wrong twice.** **`D-15`** — the macrocycle **target band is computed, stored, and never shown**: doc 22 §5's ch. 14 brief and `22c` §B2.2 both describe a visible band, but `YOUR TARGET`, the per-month rate, the rationale and the model band are hidden by **N54** (owner, 2026-07-11, pending N43/v23) on all three surfaces that would print them — the create form, the edit form, and the macro Overview's `REALISTIC TARGET` card. `planMacrocycle` still runs and `macrocycles.target_*` still persists, and the band still does two jobs the reader feels (it paces the weight climb; it is what the closeout grades against), so ch. 14 §3 documents a number **with no screen to point at** and names the two surfaces that do return it — a connected assistant, and a completed macro's `RETROSPECTIVE` band. Same finding's second half: `macro_target.present: "conservative_end"` has **no code consumer** — `COACHING_GUIDE`'s prose is its only reader, so *"you see the conservative end"* is a rule about what an AI tells you. **`D-16`** closes the thread `D-12` opened: `key_lifts.n` / `selection` are live on v25 with **no consumer anywhere in the repo**, so `22c` §C2's `KEY LIFTS` glossary recommendation is closed rather than re-sited. `22c` §B2.2 also listed the four goals as three (`CUT` missing); `22b` §4.2 corrected on both parameters. Ch. 13 is organized around **comparability** because that is what the `docs/notes/` questions are actually about — the three exclusions (deloads, backed-off sessions, mis-log outliers) with the disclosure line that explains a missing lift, then the two live surprises: sets logged in an **open** workout count immediately, and stats show each estimate **undecayed** while the prescription path fades older sessions (`22b` §5.6's PH39). **`est_strength` rendered empties `PENDING_GLOSSARY_TERMS` outright** — all fifteen keys now resolve to a `term` block, five phases before ch. 20 generates them. Live row re-read a fourth time (still v25, `91887f0f…`, hash-verified); this read produced two *absences* rather than values — `strength` is not on the row, so ch. 13's trend runs on the engine defaults (`3` · `3` · `1.5`%). 46 new `22a` rows; `GUIDE_SECTION_IDS` +11. **Phase 3h built** — chapters **16 Body data**, **17 Prescription details** and **19 Your data**, fourteen sections, 118–214 words (corpus median 209 → 203 over 92 sections), the tightest group yet. **`D-17`** — the More tab's `Log bodyweight` quick entry appends a measurement point but **does not** update `profiles.bodyweight`, while the profile editor and the day view's bodyweight chip write both; that is doc 17 §5's deliberate substrate/input boundary, but the row's label and its displayed value give a reader every reason to think the app now works from it (bodyweight-loaded movements and the target model keep reading the older profile figure). Ch. 16 §1 states the split as a three-row table and says which action serves which intent — **§8.4c rule 3 found the third writer**. Ch. 16 also carries the comparability rules as the honest reads they are: same scanner or no delta at all, and lean/fat moves under ~2 lb inside measurement noise. **Ch. 17 is written to survive either answer to O-A**: per `22b` §4.1 ②, the deterministic ask and why are documented as always rendered and the coaching line as conditional, so §4 states the doc 19 §3 *architecture* claim — the engine authors every number, the deterministic lines are a complete explanation, a coaching line is additive under its own `COACH` rule — and every sentence holds whether serving is `on` or `shadow`. Answering O-A adds a trigger-policy paragraph rather than forcing a rewrite. **Ch. 19** turns hard rule 9 into reader-facing prose (live reads with an offline screen and a retry; queued set logging, so a tap is recorded on the phone and sent when it can be) and picks up doc 23's two version surfaces per `22c` §B5.1a. 53 new `22a` rows; `GUIDE_SECTION_IDS` +14. **Phase 3i built — and Phase 3 is complete.** Chapters **18 Connecting an AI**, **20 Glossary** and **21 Troubleshooting & FAQ**, fourteen sections, 135–236 words. The User Guide is now **21 chapters / 106 sections**, median **200** words against the 350 budget, with ch. 6's mechanism section (323) still the ceiling-brusher it has been since Phase 1 — the calibration held across nine content phases and eight authoring-rule revisions. **Ch. 20 retires `PENDING_GLOSSARY_TERMS`**: §8.1's end state was enforced by a list that could only shrink, and is now enforced by an assertion that **every glossary key is filed into exactly one of chapter 20's five groups**, so a term added to `glossary.ts` and left unplaced fails CI. Grouping and one orienting line per group are authored; every definition is the app's own copy rendered at read time. **No manual-only term proved necessary** — every advanced term a chapter needed got a `glossary.ts` entry instead (`day_slot`, `load_step`). **Ch. 18 takes §8.5's one allowance for the first time**: `MCP` appears in exactly one section, the setup steps, because the app's own field reads `ADD THIS AS A CUSTOM / REMOTE MCP CONNECTOR` and a reader who has not been told the word cannot finish the step (`MAY_SAY_MCP` was written empty in Phase 2 for this case). It carries **no typed link into the AI Manual** — `ai/**` does not exist and §9.4.5 forbids authoring a link before its target resolves, a forward debt in the ch. 5 → ch. 11 pattern that Phase 6e pays. **Ch. 21 is mined, and one mined question had to be rewritten**: `22c` Part D's **F18** (*"why is the target only the low end of the range?"*) rests on a premise `D-15` disproves, so it is answered as *where is my macrocycle target*. **F11** is the one `22b` §4.3 insisted on — the 2026-08-02 re-levelling moved 9 087 historical estimates upward, and nothing else in the manual would explain a long-time user's numbers changing overnight. T-A7 and T-A8 are stated as current behavior and promised nothing; F10 links to ch. 13 rather than being answered twice. One Phase-2 search assertion **moved rather than broke**: *"estimated one-rep max"* now ranks the glossary section first, which is the right answer to a bare term search and the same alias mechanism producing it. 42 new `22a` rows; `GUIDE_SECTION_IDS` +14, now covering all 106 sections. **Phase 5 built (PR #238)** — the connector retrieval surface: `workout://user-guide-index`, `search_manual`, `get_manual_section` over the same build-time index the in-app search uses, all three release-gated on 1.1.0 with the guide routes. Built **during** the owner's Phase-4 read, which the sequencing allows — retrieval reads whatever the registry holds, so a wording change needs no change here. §10.1's no-RAG claim was **tested rather than assumed**: plain paraphrase in a reader's words (*"why did my weight go up"*, *"what does the app do with my answers"*) resolves to the section a person would have picked, on the authored `keywords` as they stood — no ranking tuned, no keyword added to pass a test, and §10.3's embeddings trigger has not fired. A **second renderer, not a second copy** (`markdown.ts`) means `get_manual_section` returns the section the reader sees — the glossary's own words for a `term`, the collapsed `detail` layer included, and the §8.2 estimate caveat carried into the payload rather than dropped on the way to a model. One test drives the gate open via `NEXT_PUBLIC_RELEASE_OVERRIDE` so the release PR is not the first thing to try the switch; `unreleased.ts` gains `connector-reads-the-guide`. D3's import guard now allows `src/lib/mcp/` with its reason (`server-only`) asserted rather than trusted. `22d` amended (+§11, ch. 4's ground truth; §8 rule 2 lifted) and **corrected** — its "5 resources after Phase 5" conflated one resource with two tools; it is 4. **Phase 6 done (PR #239)** — the **AI Manual is complete at 12 chapters / 48 sections**, mounted at `/more/connector/guide`, and `/more/connector` is reworked into its front door. The design pass (09-changelog 2026-08-13) decided there is **no second design**: D4 already made the manuals one system and everything below the routes was manual-agnostic from Phase 1, so figs 4.8/4.9/4.10 are reused at a second root, **no new figure number is claimed** and no eleventh block kind was needed; the map and chapter screens lifted into `ManualScreens.tsx` and both route trees became thin callers. Search stays **one field over both manuals** (§9.4.3) and gains the manual label on every result row, because chapter numbers restart per manual and `CH 4` had begun naming two chapters. **§7.1 is met, not approximated**: every ch. 5–8 exchange was run live on 2026-08-13, the write half as a create → capture → delete round-trip on the owner's explicit approval with `get_macrocycles` re-read afterwards (`standalone_mesocycles: []`). The live runs beat anything constructed — ch. 7's case returned `−22.7% declining/stalled` **and** matched-RIR deltas of +10.1/+10.1/+7.1/+10.5% in the same payload; ch. 6's first-written four-day split came back below MEV on seven groups; ch. 5 §1 is the **refusal** `create_macrocycle` actually returns, since it cannot succeed while an arc is live. **Two defects, both in the audit rather than the app**, both found by `22b` §9.2's verify-against-code rule: **`D-18`** — `22d` §5 said the write audit records the requesting client; the table has no client column — and **`D-19`**, which upgrades K1 from stale to **self-contradicting** (one `analyze_exercise_progress` response says "Epley-based" in `data_quality` and "Epley·Brzycki" in its own `metric_definitions`). Unplanned but earned: `GUIDE_SECTION_IDS` gained the 48 AI IDs **and a completeness test**, and ch. 18's Phase-3i **forward debt is paid**. Next: **Phase 4** — the owner's cold read, now over both manuals; it still owes a re-validation of every ledger row and of `22b` / `22c` / `22d` against the code. Then **Phase 7** (link placement, incl. the N81 inline-definition affordance). **O-A** (is `LLM_EXPLANATIONS` on or shadow in prod?) still open — it widens ch. 17 rather than blocking it **Drift pass 2026-08-10 (PR #243)** — the v26 activation invalidated four chapter ground-truth headers and five *deliberately absent* rows written from "the measuring band is inactive": ch. 8 gained the band as the boundary case past doc 21 §6.2 (`C-perex-26`/`27`), ch. 10 gained the fourth, unrated rating (`C-wt-24`), `GLOSSARY.e1rm_confidence` got its `D-14` sentence back verbatim, and `22b` §4.1 ①/§4.2/§6.3/§6.5/§7 + **O-B** were re-read against the live row and closed. Chs. 6/7 still omit the band, now on seam grounds. Recorded as `D-21` — the first case of an activation, not a code change, ageing shipped prose. **Phase 7a + 7b done (PR #244)** — deliverable **C**. The audit is [`docs/22e-link-placement-audit.md`](../22e-link-placement-audit.md), written before any code as Phase 7a requires, and the design pass (09-changelog **2026-08-15**) rules the app's definition grammar as **one system with three members** rather than three affordances — which is why **N81 was decided in the same pass**: `InfoDot` and N81's inline term are *term-level* and resolve **in place** (the reader is mid-task); a `GuideLink` is *mechanism-level* and **navigates** (the answer does not fit in a card and the reader has stopped to ask). They never substitute — an `InfoDot` on `EST. STRENGTH` says what an estimate is and never why yesterday's weight went up. The primitive is **not new drawing**: it is the app's `READ ›` / `SET UP ›` / `CSV ›` idiom, literally the line Phase 6e improvised on `/more/connector`, which now **adopts** the component — no figure redrawn, **no new figure number claimed**. Two contracts are tested rather than remembered: **the label is the destination section's own title** (so a *retitled* section fails CI, not just a renamed one) and **one release gate, inside the primitive** (nine call sites cannot forget it). **Nine placements shipped, 14 candidates declined with the earn test each failed** — the two load-bearing exclusions being *nothing on the day view's card or set grid* (N82 had just removed an icon per card; this must not spend that back) and *nothing on an unguarded form*, since a link out of a sheet holding unsaved sliders discards them. **The audit's own finding is back with the owner** (`22e` §5): the three placements rated highest on *the reader is asking right now* — the Feedback, Effort target and Load step sheets — fail **only** the unguarded-form test, and the fix already exists (`useNavigationGuard`, used by the planner board) but is a behavior change, which doc 22 §1.2 puts outside this doc's scope. Recommended as its own PR at **7c**. **7c built (PR #245) — the owner accepted §5, so the unguarded-form exclusion is *lifted rather than waived*: ten more placements ship** (the Feedback, Workout Complete, Effort target, Load step and meso Edit-details sheets, the planner's exercise sheet, both macrocycle forms, create-exercise, and the profile), design pass 09-changelog **2026-08-15 (session 2)**. The guard's reach follows the surface's exits: **pages** arm `useNavigationGuard` — which also closes a gap older than the audit, since `/cycles/new`, `/cycles/macro/[id]/edit` and `/exercises/new` have discarded unsaved input on a tab tap since they were written — while **sheets** guard the link itself, because the scrim already makes it their only exit and arming the hook would plant a history sentinel on the day view's hot path (`22e` §5.1 records the narrowing). One shared discard-confirm (`LeaveConfirm`, the planner board's sheet extracted unchanged), asserted by test. **Two audit rows were wrong and are corrected**: `/more/profile` writes on every tap and the planner's exercise sheet stages into an already-guarded board — neither was ever a form, which is why the wave is ten placements against a seven-row estimate. `22e` §4 now has **8 rows left, every one held on budget or a weak E2, none on a mechanism**. **Phase 7 is complete (PR #246, session 123)** — N81's inline term built last, as planned: six glossary entries (closing `22c` §C2's recommendation column), then `InlineTerm` over `InfoDot`'s extracted card, then eight placements split by the grammar (five inline marks, three dots) plus the manual's own `{ term }` runs. Next: **Phase 4** (the owner's cold read) and **Phase 8**. |
| N75 | **Exercise menu: "Engine audit" becomes "Prescription details", off the menu and onto the strip** (owner, 2026-08-06, Batch 32). Most people never open it, so it stopped costing a row in the ⋮ menu: the sheet is renamed and reached by tapping the prescription strip's own ask line, underlined to read as tappable without adding a chevron or a second control competing with the story. Amends **N57**, which put the drill-in in the menu *only* (the Batch-20 addendum had removed an earlier in-strip link — this restores one deliberately, on the ask line rather than beside it). Copy references updated across `prescription-narrative.ts`, `slot-effort-display.ts`, `queries/audit.ts` so one name is used everywhere | UX | MED | H | **done (PR #226)** |
| N76 | **Cycles page: show/hide completed cycles** (owner, 2026-08-06, Batch 32). Finished cycles are hidden by default — a whole closed macrocycle (status `completed`, or every block in it completed/abandoned) and a closed standalone meso; a completed meso INSIDE a running macro stays, because it is that macro's own record of progress. A discreet muted toggle sits at the bottom of the list carrying the hidden count (`SHOW 3 COMPLETED CYCLES` / `HIDE COMPLETED CYCLES`), so hidden history never reads as lost history. Implemented as `?completed=1` on the server render — no client state, no settings entry (owner: it belongs on the page) | UX | MED | G | **done (PR #226)** |
