...
| P18 | Remove the set-type option from the set menu | UX | LOW | E | **done (PR #103):** menu affordance hidden; drop-set data model (`set_type`, DROP badge) left dormant |
| P21 | Should soreness be recorded when user reports 0 days sore? | D | LOW | H | **done (PR #103, verified no-op):** current behavior already stores an explicit 0 (`soreness_days` picker gates on `=== null`, 0 is a real value through save) |
...
| PH33 | Scope admin MCP tools as private (hidden from non-admins) | F | LOW | H | **done (PR #103):** `mcp/visibility.ts` wraps the SDK tools/list handler with a `profiles.role` filter; call-time denial unchanged; fails open to visible-but-denied if the SDK internals shift |
...
| R14 | Volume counting is primary-only (1.0/0) — doc 10's fractional 1.0/0.5 + RIR≤4 hard-set rule unimplemented; false below-MEV calls, permissive MRV/ceiling. Informs I11/PH37/M8 | D→F | MED | C | **done (PR #103):** role-grain `v_meso_week_muscle_sets` (applied live) + one shared fold (`fractionalSetCount`, tunable `volume.direct/indirect`) across stats/MCP/projection/engine ceiling input; RIR≤4-or-unreported hard-set rule baked in the view (counting_max_rir/warmups_count are view-enforced doc defaults, **not** live params — recorded deviation, see PROGRESS 2026-07-02). Unblocks I11/PH37/M8/P16 |
...
| T-A1 | S1/PH39 | Reconcile the two e1RM systems; decide what each screen shows | D→F | **done (PR #103):** engine e1RM everywhere — `v_exercise_overview`/`v_meso_summary` `best_e1rm` = max stored per-set engine e1RM (applied live); `v_exercise_history.best_set_e1rm` added; stats raw-Epley paths retired (`epleyE1rm` deleted). Stats show undecayed values; decay stays prescription-only. **30-day half-life confirmed MCP-tunable** (`e1rm.recency_halflife_days` in the active v17 row). Answers PH39. |
| T-A2 | S3 | Decide + document deload handling in stats | D→B | **done (PR #103):** `getMesoProgressScores` skips deload-microcycle sessions; volume + PR stats keep deloads; denoted in the MCP metric definitions/notes. |
