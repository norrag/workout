...
| N44 | Prescription detail: show the prescribed/target e1RM, not just the anchor (owner, Batch 17). **Shipped** as the sheet's `EST. STRENGTH (e1RM)` block: PRESCRIBED IMPLIES (implied e1RM via `estimateE1rm` off the sheet's own fields, effective load for bodyweight) + TARGET ANCHOR A\* (`prescription_anchor`, stepped rows) + N45's MEASURED ANCHOR row. `explain_prescription` needed no change — the decision `inputs`/trace it already returns carry the anchor + A\*, and now the N45 source too | F | MED | P | **done (PR #<n>)** — 09-changelog 2026-07-12 entry |
| N45 | Prescription detail: show the coordinate the anchor was derived from (owner, Batch 17). **Shipped:** `recencyWeightedE1rm` returns `source` (winning set weight/reps/ageDays/sessionKey; `mean` reports its highest-value sample) → widened on the `inputs.strengthAnchor` schema as `.nullish()` (stored decision inputs parse byte-identically; anchor already on the doc-14 §3 denylist ⇒ fingerprint-neutral, pinned in `fingerprint.test.ts`) → `anchors.ts` enriches `performedAt` → `LoggedExercise.e1rm_anchor_source` → MEASURED ANCHOR row with the `115 × 11 ON JUN 28` coordinate; newly recorded decisions + `explain_prescription` carry it for free. W·D coordinate deliberately skipped (needs a workouts→microcycles join; set+date is exactly how the owner phrased the PH39 complaint) — revisit only if asked | F | MED | P | **done (PR #<n>)** — engine provenance tests + golden updated |
