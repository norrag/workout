...
| I12 | Address mesocycle management under a macrocycle | F | HIGH | D | **done (PR #137)** — the in-app surface now covers the whole MCP authoring set. PR #92 (MCP), PR #134 (duplicate + proactive START gate), PR #137 (owner authorized Claude's design 2026-07-03): **Place into macrocycle** sheet (fills-or-appends, exact landing stated per row), **Edit details** sheet (name any time; weeks/RIR/deload until start), **BLOCKS** management on the macro edit page (▲▼/✕/+ ADD), **WEEKLY SETS PER MUSCLE** live readout on the planner board (shared R14 fold, MEV/MRV bands). Design of record = 09 2026-07-03 session 4. Deliberately MCP-only: explicit-position placement, phase editing |
...
| N13 | Reset-to-prescription broken on the exercise's first set (regressed near the N5 swap fix; first-set prescription correctness after swaps + resets must be right) | B | HIGH | G | **done (PR #137)** — root cause was R13-era, surfaced by exercising the flow post-N5: the reset echo (`set_weights` cleared → plannedWeight null) arrived through the planned-input channel, whose typed-row guard never releases on an unlogged row — and set 1 is necessarily typed-in (that's what makes the reset option appear). The override-CLEARING transition is now classified `prescription-reset` (always adopts, clears the typed flag); already-null transitions keep the R13 mid-typing protection. Swap path (N5 remount key) verified intact. +1 unit test |
