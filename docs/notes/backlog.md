...
| R5 | Completion lock bypassable/asymmetric (completed→in_progress flip; `workout_exercises`/`workout_feedback` mutable post-completion; sets insertable into completed workouts) + child INSERT policies skip parent-ownership (feedback-slot squat) | B | MED | K | **done (PR #116)** — migration `20260702000006`: workouts update-open/insert-planned-own-micro/delete-planned-no-history; workout_exercises + workout_feedback + logged_sets INSERT + exercise_feedback INSERT/WITH CHECK all gated on owned, planned/in_progress parents (squat closed); microcycles update-not-completed/insert-own-meso/delete-no-history. Full authed write-path inventory first (nothing legitimate blocked — completion writes land pre-flip); 29 scratch probes + 9 new RLS suite tests |
| R7 | Service worker caches authed pages/RSC ~24h, never purged on sign-out (stale prescriptions offline + shared-device privacy; contradicts online-only) | B | MED | K | **done (PR #116)** — `sw.ts` runtime caching trimmed to static assets only (hashed `/_next/static`, same-origin images; documents/RSC/`/api/` NetworkOnly); offline navigations get a precached `/~offline` ledger interstitial instead of stale prescriptions; auth screens purge non-precache CacheStorage + the `lastWorkoutId` pointer (`ClearClientCaches`) |
