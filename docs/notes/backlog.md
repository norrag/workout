...
| R23 | Repo hygiene: 2 unused-but-live server actions (attack surface), dead exports/components (NumberStepper stale-closure), dead `v_muscle_group_volume` view, dep nits (analyzer major, tsx, dependabot) | F | LOW | L | **done (PR #122)** — migration `20260703000002` applied live; `v_meso_week_sets` retirement stays with R21's golden-path rework (noted there) |
