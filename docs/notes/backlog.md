...
| PH29 | Page switches slow + double-layer label/loading glitch; want instant switch w/ placeholders | B | HIGH | G | **done (PR #84)** — double-marker glitch fixed in BottomNav (active+pending no longer both draw ■). The *instant-switch/slowness* remainder is server-compute-bound (Workout tab) → folded into N1/WS-J perf pass. |
...
| PH34 | Meso-stats "planned sets" review — what counts as "planned"? completed + remaining prescribed | Q→B | MED | C | **needs-input (decision framed):** confirmed a real bug — future weeks are materialized lazily (only after the prior week's same day completes), so `v_meso_week_sets` has no rows for them; the stats UI silently substitutes the static planner baseline (`initial_sets`, ignores autoregulation) while the MCP tool reports `null` — the two shared surfaces disagree (CLAUDE.md "one definition of progress" violation). Fix needs an owner call on what "planned" means for an unmaterialized week: (a) static planner baseline, (b) autoregulated **projection** (needs a new batch projector generalizing `projectNextPrescription`), or (c) just count materialized weeks (mark the rest "not yet planned"). Then a new append-only view migration + RLS, read by both surfaces. Detail in agent investigation; see `scoping.md` PH34. |
| PH36 | Check model & weight-increment settings for bodyweight-only exercises | B/Q | MED | F | **done (PR #84)** — engine half already correct under v16 (reps-only at fixed bodyweight; increment inert); UI gap closed by hiding the Load-step control for bodyweight_only lifts. |
| PH37 | Aggregate strength gains per muscle group over macro/meso/all-time | F | — | C | inbox |
| PH38 | First sets/reps wrong when you switch exercise (correct after reset to prescription) | B | HIGH | G | **done (PR #84)** — root cause: `replaceWorkoutExercise` carried the outgoing exercise's `set_weights` overrides onto the swap; now cleared so the incoming exercise starts from its own seed. |
