...
| N7 | Day-view note sheet: after keyboard + sheet dismiss, page scroll lands LOWER than where the user started. Root cause: shared `useScrollLock` never saves/restores `scrollY` (body `overflow:hidden` doesn't pin offset on installed iOS PWA). **Fixed:** lock captures `scrollY` + applies `position:fixed; top:-scrollY; width:100%`; release restores styles and `scrollTo`s the saved offset — one file, covers every sheet/menu. Owner on-device (installed PWA) spot-check pending | UX | MED | E | done (PR #__N__) |
| N8 | Cycles meso badges: planned mesos show a **white "PLANNED" text badge** (CURRENT's style, white), checkbox reserved for **completed** only, unplanned keep `+ PLAN`; only current/completed render full ink — planned + unplanned stay muted. **Shipped on both surfaces** per the 2026-07-03 addendum: `/cycles` `StatusMark` planned → ink PLANNED badge (CURRENT geometry; ink = "white" under the dark inversion) + muting widened to planned; macro timeline planned rows swap the progress bar for the badge, numbered marks stay. 09 dated delta recorded (figs 2.1/2.2) | UX | HIGH | D | done (PR #__N__) |
...
| N11 | Deload sets show ▼ underperform arrow even at exactly-prescribed weight+reps. Root cause: RIR-asymmetric e1RM comparison — prescription side bakes in the week's target RIR (deload ≈ 6, the max), logged side gets `rir_reported: null` → treated as RIR 0. **Fixed:** marker extracted to pure `day-rules.ts::loggedSetMarker`, compares both sides at the same RIR when unreported (`rir_reported ?? targetRir`); 6 unit tests incl. the deload regression. Working weeks' smaller skew fixed by the same rule | B | MED | G | done (PR #__N__) |
