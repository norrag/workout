...
| N53 | No launch splash any more — long black screens; "invest the time to get this right" (owner, Batch 17, HIGH). Splash code unchanged since PR #119 — not a regression. Two stacked causes: (1) black-not-cream — every launch surface is theme-aware and flips to near-black `#14110C` under OS dark mode (splash renders but reads as a black screen), though dark mode is out of scope per doc 08; (2) long — the pre-document window the in-app splash can't cover: middleware blocks on `auth.getUser()` before HTML streams, then two more serialized `getUser()` calls, with the SW shell deliberately NetworkOnly. Fix: force launch surfaces cream per the light-only design + collapse the auth round-trips; confirm device appearance. Relates N1 (WS-J perceived-speed north star) | B+UX | HIGH | J | **triaged** — causes + fix directions in [`scoping.md`](./scoping.md#n53--no-launch-splash-long-black-screens--bux--high--medium) |
