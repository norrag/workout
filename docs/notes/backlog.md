...
| I12 | Address mesocycle management under a macrocycle | F | HIGH | D | **advanced (PR #92 MCP, PR #134 in-app):** MCP authoring shipped 2026-07-01. In-app delta **fully scoped 2026-07-03** (`scoping.md` § I12 gap table — the helpers all exist; remaining work is pure UI) + first two slices shipped in PR #134: ⋮ menu **Duplicate mesocycle** and the **proactive activation gate** on START (disabled + reason instead of failing on tap). Remaining (need a 09 design delta or owner input — no mockup figures): attach-into-macro picker (+ seed slot from template/copy), meso header-edit after finalize, direct slot add/remove/reorder, plan-time volume preview |
