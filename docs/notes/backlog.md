...
| N6 | Pull-to-refresh on at least the day view + cycles pages/subpages (installed PWA = no native PTR). **Built:** one shared `PullToRefresh` wrapper in `(app)/layout.tsx` (document scrolls; covers every page) — gesture armed at `scrollY === 0` only, resisted pull, `router.refresh()` in a transition, travelling-gap square indicator; `overscroll-behavior-y: contain` prevents Android double-fire. Owner on-device gesture check pending | F | MED | E | done (PR #132) |
| N9 | Macro Performance tab reorg: **muscle-group strength gain primary**, each group expandable to the exercises that rolled into it; flat per-exercise list dropped at macro scope. **Built:** `rollupMuscleProgress` carries `contributors[]`; new `MuscleStrengthSection` (▸/▾ drill-down: first→last e1RM, sessions, SECONDARY marker) on the macro Performance panel; meso tab's `StrengthProgressSection` untouched; MCP unaffected. 09 delta recorded (2026-07-03 session 2) | F | HIGH | C | done (PR #132) |
| N10 | Meso Performance tab trim: "TOP SET BY WEEK — KEY LIFTS" + "ACROSS MACRO" sections **removed** (macro-scope content on a meso view) — `buildKeyLifts`, top-set fold, chart query block + types deleted (~230 lines net); `contextLine`'s `MESO n OF m` re-derived from the macro's meso ordering (no longer coupled to `keyLifts[0]`, and now shows even without a key lift). Shipped with N9 | F | HIGH | C | done (PR #132) |
| N12 | Set logging takes seconds; spinner occasionally never resolves. **Built (WS-J slice):** latency — `logSet`'s 4 serial stamp SELECTs → 1 embedded read (live-validated), `in_progress` flip skipped when already past planned, reconcile-gate watermark reads **closed workouts only** so the first set no longer busts it; hang — spinner tracks the server action (15s watchdog) not the revalidation commit, box acknowledges on write-confirm (`ack` + row-remount echo), timeout = retryable shake+toast (R3 upsert converges). J-Phase-2 #5 still deferred (needs #7 tagging); #6 assessed — day view consumes ~all columns, narrowing buys bytes not round trips. Owner on-device feel check pending | B | HIGH | J | done (PR #132) |
