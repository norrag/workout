...
| N74 | **User Manual + AI/MCP Manual** (owner task, 2026-08-05). Two user-facing manuals plus a follow-on link-placement pass: (A) a **User Guide** — key terms, how to use every screen, plain-language explanations of how the load-bearing machinery works (the strength anchor, the Epley/Brzycki e1RM average over effective reps and the role e1RM plays, volume counting, autoregulation) with the *reasoning* behind each method, indexed/searchable/navigable; (B) an **AI Manual** living under `/more/connector` — setup, the concept, the 56-tool surface (17 admin-gated), and worked use cases for macrocycle creation/management, mesocycle creation/management, performance analysis, and coaching; (C) afterward, strategic insertion of deep links from dense app surfaces into the exact manual section that explains them. Plan + IA + content contracts + phases: [`docs/22-user-manual.md`](../22-user-manual.md) — Phase 0 is a four-part ground-truth audit (doc supersession map, route-by-route functional inventory, concept/FAQ inventory, connector inventory) that **gates all prose**, because the docs supersede each other in place (09↔08↔06, 19↔18, 21↔11, 16↔17) and two param versions shipped **inactive** (v20/v23) — a manual written from spec prose alone would document behavior that isn't live. Key structural calls: content as a **typed block model** (no new dependency; feeds the renderer, the search index and an optional `workout://user-guide` MCP resource from one source), a **claims ledger** (`22a`) mapping every factual assertion to code so the manual is maintainable rather than a snapshot, and three CI-enforced content contracts (glossary identity with `glossary.ts`, the doc-10 §9 honesty guardrails, link-target validity). Extends **N25** (archived — `InfoDot` + glossary is the term-level layer; this is the mechanism-level layer above it, sharing one glossary). Open owner questions: doc 22 §12 (O1–O6) | F | — | M | **plan written — awaiting owner review of [doc 22](../22-user-manual.md) §4 decisions + §12 questions before Phase 0** |
