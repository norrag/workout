...
| R13 | SetRow re-sync effect clobbers in-progress typing after background weight writes / auto-match — wrong reps can get logged | B | MED | G | done (PR #100) |
...
| R18 | Modal a11y + touch: no focus management/Escape/trap on any sheet/menu; LOG checkbox 21px & ⋮/▲▼ below-minimum targets; `maximumScale:1` kills pinch-zoom. Scopes doc-07 Phase-7 a11y audit | UX | MED | E | done (PR #100) — full doc-07 Phase-7 audit of the logging flow still open as its own phase item |
| R19 | Small defect sweep: no `not-found.tsx` (Workout tab can 404 via stale `lastWorkoutId`); CompleteSheet totals contradict header progress. ~~SAVE AS TEMPLATE SubmitButton~~ shipped with R17 (PR #98) | UX | LOW | E | done (PR #100) — all 3 bullets closed |
