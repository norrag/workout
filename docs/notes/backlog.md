...
| N47 | Tab bar detaches from the bottom on scroll and goes dead; relaunch cures it (owner, Batch 17, HIGH; screenshot referenced but not attached). Ancestor-transform theory ruled out (no ancestor of the fixed `BottomNav` ever gets a transform; PullToRefresh animates height and is a sibling). Top hypothesis: `useScrollLock` toggling `body{position:fixed}` on every overlay open/close leaves fixed elements bound to a stale viewport on iOS standalone — matches all three symptoms (rides up mid-screen, taps dead, only relaunch resets). Fix: move the scroll lock off body-position-fixed; verify on device. **Screenshots received (Batch-17 addendum, [`assets/`](./assets/)):** detach reproduced on /cycles AND the planner board; the fixed bar renders as if the viewport bottom were mid-screen (content flows below it), and the planner capture is the same minute as a create-meso BottomSheet + keyboard session — corroborating the scroll-lock (+ keyboard visual-viewport) trigger | B | HIGH | G | **triaged** — hypothesis corroborated by screenshots ([`scoping.md`](./scoping.md#n47--tab-bar-detaches-from-the-bottom--goes-dead-ios-standalone--b--high--medium)); build the scroll-lock rework + verify on device |
| N48 | Filters in the replace-exercise sheet (owner, Batch 17). `ReplaceSheet` has text search + a static muscle chip only; candidates already carry `equipment_type`, so an EQUIP `FilterBar` axis drops in exactly as the planner picker does (N29 grammar). Fold `AddExerciseSheet`'s pre-N29 hand-rolled chips onto `FilterBar` in the same pass. Ship with N49 (same sheet) | UX | MED | E | **ready** — scoped in [`scoping.md`](./scoping.md#n48--filters-in-the-replace-exercise-sheet--ux--small) |
| N49 | Replace exercise commits on a bare tap; needs a confirm button (owner, Batch 17). The day-view `ReplaceSheet` is the only picker that tap-commits — the planner `ExercisePicker` (N31 spec) and the day-view `AddExerciseSheet` both use select-then-confirm with a disabled-until-valid button. Mirror the planner pattern. Ship with N48 | UX | MED | E | **ready** — scoped in [`scoping.md`](./scoping.md#n49--replace-commits-on-a-bare-tap-needs-a-confirm-step--ux--small) |
| N50 | Past workouts: weight/rep boxes editable but edits silently don't save — lock them (owner, Batch 17). Root cause: the `staticCells` gate omits the existing `readOnly` flag (completed/skipped), so logged rows on completed workouts render live inputs whose blur-save the completion-lock RLS silently no-ops (server side already correct per R5 hardening). One-conditional client fix | B | MED | G | **ready** — scoped in [`scoping.md`](./scoping.md#n50--lock-past-workout-weightrep-inputs--b--small-near-trivial) |
| N51 | New-meso seed prescribes reps below the target window — 6 when the window is 8–12 (owner, Batch 17). Mechanism confirmed: `seedCore` prices `target_low` at start RIR, rounds the load to the *nearest* increment (up half the time), re-predicts reps at the heavier load, and clamps only to the hard floor `min=6` — never calling `boundRepsToWindow`, the exact step-the-weight-down rule the working prescribe path applies. Cold-start/swap-in seed branch has the identical defect. Fix both branches + engine unit/golden tests (hard rule 3) | B | HIGH | G | **ready** — root cause + fix in [`scoping.md`](./scoping.md#n51--new-meso-seed-prescribes-below-the-rep-window-6-when-812--b--high--small-fix--engine-tests) |
...
| N53 | No launch splash any more — long black screens; "invest the time to get this right" (owner, Batch 17, HIGH). Splash code unchanged since PR #119 — not a regression. Two stacked causes: (1) black-not-cream — every launch surface is theme-aware and flips to near-black `#14110C` under OS dark mode (splash renders but reads as a black screen), though dark mode is out of scope per doc 08; (2) long — the pre-document window the in-app splash can't cover: middleware blocks on `auth.getUser()` before HTML streams, then two more serialized `getUser()` calls, with the SW shell deliberately NetworkOnly. **Dark appearance CONFIRMED by the Batch-17 screenshots** (the whole app renders dark on the owner's device), so cause (1) is established, not hypothesized. The pre-paint collapse (auth round-trips) is unconditional; the splash-color half needs an owner call since the app itself renders dark after launch: (a) force ALL launch surfaces cream per the light-only design (doc 08 — but then cream splash → dark app flash), (b) keep theme-aware but make the dark splash *legible* (visible logotype/brand on `#14110C` so it reads as a splash, not a dead screen), or (c) retire the dark theme entirely (doc 08 calls dark mode out of scope, yet a full dark theme ships today). Relates N1 (WS-J perceived-speed north star) | B+UX | HIGH | J | **needs-input** — pre-paint fix ready; splash-color option (a)/(b)/(c) framed in-row, causes in [`scoping.md`](./scoping.md#n53--no-launch-splash-long-black-screens--bux--high--medium) |
| N54 | Disable the macro goal-target estimates again for now (owner, Batch 17 — decision in the note itself). Re-enabled by PR #178; the original N21 hide was a plain JSX edit, no flag. Disable = revert the #178 JSX hunks (overview REALISTIC TARGET card; create-flow YOUR TARGET + MODEL BAND priming row — keep LAST BLOCK MEASURED, it's measured) + include the third surface the note didn't name (goals-edit YOUR TARGET, pre-#178) for consistency. Pure view change; re-enable when N43's v23 makes the band trustworthy | F | MED | C | **ready** — scoped in [`scoping.md`](./scoping.md#n54--disable-the-macro-goal-target-estimate-cards-again--f--small--owner-decided); owner-decided |
| N55 | Create-meso sheet: "WEEKS — INCLUDING DELOAD" label contradicts the unchecked "Final week is a deload" option (owner, Batch-17 addendum, annotated screenshot). The label is hard-coded in the planner's create sheet (`PlannerBoard.tsx:1318`) while `ramp.deload` is toggleable (`:1429-1441`); the edit-details sheet already renders the suffix conditionally (`MesoHeader.tsx:605`). Mirror that conditional (and its summary line `:1353` already handles the deload suffix correctly) | UX | LOW | D | **ready** — trivial; scoped in [`scoping.md`](./scoping.md#n55--create-meso-weeks-label-claims-a-deload-the-checkbox-controls--ux--trivial) |
