...
| R3 | Non-atomic plan/param writes: `saveMesoPlan` delete-then-insert can wipe an (active) plan — reachable from MCP `edit_mesocycle`/`create_mesocycle` input-validation gaps (PR #92 surface); `startMeso` half-apply = permanently unstartable meso; `generateDay` poisoned empty day; `activateEngineParams` can leave ZERO active params app-wide; missing uniques let races duplicate days/sets | B | HIGH | K | **done (PR #110):** `save_meso_plan`/`activate_engine_params`/`insert_generated_day` DB functions (one transaction each; migration `20260702000005` applied live + verified), `startMeso` retry-safe (recorded deviation: seed math stays pure-TS per rule 3), both unique keys added (live logSet retry-storm dupes deduped — 15 rows, recorded rule-5 deviation), `logSet` upsert, MCP plan validation before any write + orphan-draft compensation; poisoned empty days now heal via `planCatchUp` |
| R4 | Plan regeneration cascade-deletes logged history (`regenerateOpenWorkouts` skips the logged-set check; `logSet`'s status flip silently swallowed → hard-rule-#5 breach path) | B | HIGH | K | **done (PR #110):** both delete branches exclude anything with logged sets (pure `withoutLoggedHistory`, `removeWorkoutExercise` pattern); `logSet` status-flip error surfaced; `completeWorkout` statuses batched + error-checked |
