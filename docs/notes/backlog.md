...
| N74 | **Production schema drift took out week generation for two days — plus the week-boundary/skip gaps it exposed** (owner, 2026-08-04, Batch 32, HIGH). Reported as "doing a week's workouts out of order created an odd state": Parrish completed W1 D3→D1→D2, and afterwards no W2 prescriptions existed ("not planned yet"), the Workout tab read *"Every workout this week is logged. Next week's targets generate when the engine runs."*, and an MCP day reorder returned an internal database error. **Out-of-order was not the cause and is not a defect** — progression is day-slot keyed (W(N+1)·D advances from W(N)·D), each completed day generates its own counterpart independently, and the week-close branch is order-insensitive; the DayView navigator and cycles grid already treat any day of the active week as reachable. **Actual cause: `20260802000004_slot_rep_position.sql` was never applied to hosted.** PR #221 merged code reading `meso_exercises.rep_position` on 2026-08-02 22:15 UTC; `getSlotEffortRows` names that column in its `select` *and* its `.or()` filter, so every call raised 42703 — killing `advanceWeekAfterWorkout`, `reconcilePrescriptions`/`loadMesoStaleInputs`, `catchUpProgression` and the MCP plan surfaces. All four sites are deliberate degrade-gracefully catches, so a total outage rendered as reassurance. Blast radius: **Parrish and Sara fully stuck** (meso active, no active micro, next week never generated), Madeline and Garron mid-week and would have hit the same wall. Migration applied to hosted 2026-08-04 via MCP (`20260804143526 / slot_rep_position`), verified; both stuck users self-heal on next app open via `catchUpMesoGeneration`. Four follow-on fixes shipped with it: (1) **migration-drift guard** — every other CI job applies migrations to a throwaway local stack and so can only prove the repo self-consistent; `npm run db:check` compares repo stems against hosted `schema_migrations` (by *name stem*, since the hosted version is assigned at apply time and the `name` column is recorded inconsistently), baselining the two pre-tracking migrations so the output stays actionable; (2) **schema drift re-scoped in the report funnel** (42703/42P01/42883 → `schema-drift:<scope>` + a `remediation` hint) so a self-inflicted outage stops reading as flaky infrastructure; (3) **the Workout-tab dead end replaced** — an active meso with no next workout is never a normal resting state, so it now says so and offers a retry (`retryWeekGenerationAction`); (4) **`skipWorkout`** — a whole-day skip, the terminal state an untrained day never had, so one dropped session can no longer leave a week un-closable and the next week ungeneratable (reachable by ordinary use, no drift required). Plus the **week-boundary gate**: a W(N+1) day materialized by an early W(N) completion was deep-linkable *and loggable* from the cycles grid; it is now fully viewable but not loggable until its week activates (`isWeekLocked`, enforced in `logSet`, not just the UI). Ordering model confirmed by the owner: **free within a week, gated at the boundary.** Relates: N70/PR #221 (the unapplied migration), N68 (the queue that bounds the refusal), R20 (degrade *loudly*), R3 | B | HIGH | J | **done (PR #222)** |
