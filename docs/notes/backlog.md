...
| N67 | **Temporary per-exercise effort/load management** (owner, 2026-07-31, Batch 28 + 28b). Origin: a live lumbar-nerve episode — coach and owner agreed a rehab plan (stop deadlifts, cut related workload, substitute safer movements) and there was no way to write it down. **Direction 1 — coach-authored prescription overrides via MCP (PARKED 2026-07-31, same day):** review [`docs/reviews/2026-07-31-coach-override-prescriptions.md`](../reviews/2026-07-31-coach-override-prescriptions.md) — owner judged it "messy and a large paradigm shift". Findings kept: the engine anchors on what was PERFORMED so five couplings carry the work in regardless of storage (§2); a display-only override layer desyncs every volume view (§3); `mesoStaleSignature` has no clock (§4.1); the `LOOKBACK_WEEKS = 2` substitution cliff (§4.4 — still the only record of that failure mode); no separate coach principal (§5); reduce-only safety bounds (§6). **Direction 2 — exercise-level RIR assignment (LIVE):** owner's alternative — per-exercise (and per-week) target RIR, managing effort through the existing RIR framework; no windows, no clock, no new paradigm. Assessment: [`docs/reviews/2026-07-31-exercise-level-rir.md`](../reviews/2026-07-31-exercise-level-rir.md). Much cheaper than direction 1 and mostly already built — `workout_exercises.target_rir` **already exists per slot** and the anchor already reads it (`queries/anchors.ts`), `meso_exercises` is the slot-grain plan row, `mesocycles.rir_schedule` (N18-B) is the per-week authoring precedent, `edit_mesocycle` is the MCP seam, and doc 14 §7 makes invalidation mechanical. Engine change of substance: `prescribe()` always returns `targetRir: inputs.week.targetRir`, so the per-slot column is a copy today; it must become a resolved input, applied AFTER `liveWeekRirUpdates`. Three caveats: **(a) the metric benefit is blocked on N68** (the stats stamp ignores prescribed RIR — the headline claim fails without it); **(b) RIR is a ~2 %/step lever** — RIR 1→5 at 9 reps is only **−9.1 %** load (§4), excellent for fatigue, weak for absolute spinal/joint load; the app's own deload pairs RIR 6 with a 50 % set cut, so a per-exercise **set floor** should ride along; **(c) it cannot express "stop deadlifts"** — bounded substitution stays open (direction 1 §4.4). Recommended shape: **floor semantics** (`max(weekRir, floor)`, reduce-only, deload wins) + a `reason` column + an earn-gate predicate. Eight owner decisions in the assessment §9. Relates: **N68** (blocking), N38 (honest-RIR capture — its value rises), N39 (per-exercise progression-off — a floor makes it nearly redundant), N18-B, doc 14 §7, doc 16 §3.4, doc 19 | F | HIGH | P | **needs-input** — assessment shipped (PR #211); blocked on the §9 Q1–Q8 decisions, and the build sequence starts with N68 |
| N68 | **The app has two different RIR assumptions — stats stamp vs strength anchor** (found 2026-07-31 while assessing N67 direction 2; `B`, HIGH). The doc-11 premise is that a logged `weight × reps` against a *prescribed* target RIR is itself an RIR data point. The **anchor** implements it (`queries/anchors.ts` reads each set's `workout_exercises.target_rir`); the **stored per-set stamp does not** — `computeSetE1rm` (`app/(app)/log/actions.ts:86`) uses `logged_sets.rir_reported`, which the app **never writes** (`DayView.tsx:1698` hard-codes `null`, and no capture surface exists — N38), so `effectiveReps = reps + 0` and **every logged set is stamped as if taken to failure**. Everything downstream reads that stamp: `v_exercise_history.e1rm` + `.best_set_e1rm`, `v_exercise_overview.best_e1rm`, `v_meso_summary.best_e1rm`, `v_exercise_prs`, the strength trend, MCP history. This is the general form of the 384-vs-367.5 divergence diagnosed as a formula-version drift in [`2026-07-04-swap-prescription-provenance.md`](../reviews/2026-07-04-swap-prescription-provenance.md) §8.2 — the deeper cause is that the two paths disagree on the *input*. **Blocks N67 direction 2**: prescribe RIR 5, the athlete complies, and the history chart still plots the 9 %-lighter work as a strength decline. **Fix:** stamp `rir_reported ?? workout_exercises.target_rir` and backfill through the existing `queries/e1rm-restamp.ts` hook. **Blast radius (own PR, own doc-10/11 amendment):** every historical e1RM moves **up** (a set prescribed at RIR 2 gains 2 effective reps) — PRs, `best_e1rm`, key lifts and the strength trend all re-level once, visibly. Also raises the value of N38 (the further prescribed RIR sits from 0, the more the metric leans on an unverified premise). Analysis in [`docs/reviews/2026-07-31-exercise-level-rir.md`](../reviews/2026-07-31-exercise-level-rir.md) §3 | B | HIGH | A | **triaged** — fix scoped; owner decision on the re-levelling is §9 Q1 |
...
### Batch 28 — coach-authored prescription overrides (2026-07-31, session task)
...
> implement."
> *[→ N67; review doc `docs/reviews/2026-07-31-coach-override-prescriptions.md`,
...
> *[→ N67 direction 2 (the override review is PARKED, not deleted — its §2/§4.4
> findings still bind); assessment
> `docs/reviews/2026-07-31-exercise-level-rir.md`. Spawned **N68** — the stats
