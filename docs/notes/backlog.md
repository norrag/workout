...
| N67 | **Exercise-level RIR — temporary per-exercise effort/load management** (owner, 2026-07-31, Batches 28/28b/28c). Origin: a live lumbar-nerve episode — a coach-agreed rehab plan had nowhere to live. *Direction 1 (coach-authored prescription overrides) **CLOSED** per A8* — [`reviews/2026-07-31-coach-override-prescriptions.md`](../reviews/2026-07-31-coach-override-prescriptions.md), kept as history; its findings went to doc 21 §5/§8 except the substitution cliff → **N69**. *Direction 2 (**LIVE**): per day-slot × exercise target RIR, per week.* Assessment [`reviews/2026-07-31-exercise-level-rir.md`](../reviews/2026-07-31-exercise-level-rir.md) → **owner decided A1–A8 (2026-07-31)** → authoritative build spec **[`docs/21-exercise-level-rir.md`](../21-exercise-level-rir.md)** (6 phases, §10). Settled: **absolute** semantics (set wins, unset yields to the ramp — floor semantics rejected as extra parameters); **repricing = the deload's own mechanic** (window-centered reps at the target RIR off the anchor, no climb that week) — which also fixes the incoherent-triple case; grain = `meso_exercises` + per-week array (mirroring `mesocycles.rir_schedule` incl. orphan-clearing); a **per-exercise set cap** ships too (MCP-first, minimal UI); earn-gate predicate + miss-throttle parity; `reason` column; RIR ceiling **stays 8** (even RIR 8 is only −14.6 % load — beyond that the lever is sets/substitution). Correction recorded for the build: the owner's "floor reps" would price the backed-off load *heavier* (fewer effective reps ⇒ higher weight); centered is both lighter and deload-parity (doc 21 §4.2 table). Two confirmations still open in doc 21 §9 (volume counting; per-set capture ergonomics). Relates: **N68** (Phase 1), **N38** (absorbed by Phase 1), **N69** (out of scope), N39 (redundant under an assignment), N18-B, N25 (RIR copy), doc 14 §7, doc 16 §3.4, doc 19 | F | HIGH | P | **ready** — spec'd (PR #211); owner begins phased implementation in a new session, starting with Phase 1 |
| N68 | **The app has two different RIR assumptions — stats stamp vs strength anchor** (found 2026-07-31 assessing N67; `B`, HIGH). The anchor honors each set's prescribed `workout_exercises.target_rir` (`queries/anchors.ts`); the stored per-set stamp uses `logged_sets.rir_reported` (`app/(app)/log/actions.ts:86`), which the app **never writes** (`DayView.tsx:1698` sends `null`) ⇒ `effectiveReps = reps + 0` ⇒ **every stats surface treats every set as taken to failure** (`v_exercise_history.e1rm`/`.best_set_e1rm`, `v_exercise_overview.best_e1rm`, `v_meso_summary.best_e1rm`, `v_exercise_prs`, strength trend, MCP history). General form of the 384-vs-367.5 divergence ([2026-07-04 review](../reviews/2026-07-04-swap-prescription-provenance.md) §8.2). **Owner decision A1 widened the fix**: rather than only falling back to the prescribed RIR, logged sets now **capture `rir_reported` per set** and stats read RIR + report effective reps, with one resolution `rir_reported ?? target_rir` shared by the stamp, the anchor and the marker — which also absorbs **N38** (honest-RIR capture) and amends the doc-11 premise (the prescription becomes a *suggestion*; report honest reserve even when it differs). Guard pinned: the capture default is the prescribed RIR, **never 0** — that is the N11 regression (`day-rules.test.ts:114`). Backfill via the existing `queries/e1rm-restamp.ts`; **every historical e1RM re-levels upward once** (PRs, best_e1rm, key lifts, strength trend), so it ships in its own PR with a doc 10 §9 note. Spec: [doc 21 §2 / Phase 1](../21-exercise-level-rir.md) | B | HIGH | A | **ready** — Phase 1 of doc 21; blocks N67's metric benefit |
| N69 | **Bounded exercise substitution + the `LOOKBACK_WEEKS = 2` return cliff** (spun out 2026-07-31 when N67 direction 1 closed; `F`, MED). The one clause exercise-level RIR cannot express: "stop deadlifts for two weeks, do RDLs instead, then come back". Today a **single-session** swap exists (`replaceWorkoutExercise` → `queries/slot-prescription.ts`) and a **plan-wide, all-future-weeks** swap exists (`edit_mesocycle { op: "swap_exercise" }`) — but nothing bounded to a window. Worse, the return is mis-priced past two weeks: `LOOKBACK_WEEKS = 2` (`queries/slot-prescription.ts:70`) means a 3-week substitution brings the original back as a **cold seed off the prior peak** (`v_exercise_prs`) — the worst direction after an injury. Only written record of the failure mode: [`reviews/2026-07-31-coach-override-prescriptions.md`](../reviews/2026-07-31-coach-override-prescriptions.md) §4.4 (closed doc, kept for this). Options framed there: extend the lookback while a substitution is active, or (recommended) require the return to be prescribed rather than inferred. Any MCP surface inherits that doc's §5/§6 constraints (no separate coach principal; reduce-only bounds). Relates: N33 (the lookback's origin), N67 | F | MED | P | **triaged** — deferred until doc 21 lands; revisit if a real substitution runs past 2 weeks |
...
> A7: Sure, yes.
> A8: Close it."
> *[→ N67 decided; build spec `docs/21-exercise-level-rir.md` (6 phases). A1
> widened N68 into full per-set RIR capture and absorbed N38; A8 closed the
> override review and spun out N69. Two corrections recorded in the spec: "floor
