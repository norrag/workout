...
|----|-------|------|-----|----|--------|
| N91 | **A set with no weight logged as a real 0 lb set** (found 2026-09-01, driving the built app against a local stack — not an owner note). On a slot with nothing on record to price it — no prescription, no planned weight, no history, i.e. **every set of a movement's first session** — `initialWeight` fell through to `?? 0` and the `LB` cell rendered a bold `0`. The box was tappable, so one tap wrote `logged_sets(weight: 0, reps: 8)`: a session in the count, sets against the muscle's weekly volume, and a row the anchor query has to reason about, all from a tap the lifter meant as "record what I just did". Reproduced end to end (the DB row is `weight 0, reps 8, e1rm NULL`). Two aggravating halves: **(a)** the same fall-through applied to an emptied cell, because `Number("")` is **0, not NaN**, and `save()` only guarded `NaN` — so clearing `LB` and tapping `LOG` wrote a 0 too, on any exercise, at any time; **(b)** the day view's own *future* rows already render `—` for exactly this state, so the editable row was the odd one out. Fixed: `initialWeight` resolves to **null** and every re-sync goes through one `initialWeightText` (the two `useEffect` re-syncs were putting the `0` back), the cell carries a `—` placeholder, and `day-rules.ts::unloggableSetReason` refuses the write and names the cell it needs — exempting `bodyweight_only`, whose load is the lifter's bodyweight and whose cell is read-only. Server validation left at `min(0)` deliberately: historical rows may exist | B | HIGH | E | **done (PR #TBD)** |
| N92 | **The app's secondary text tone failed WCAG AA on every screen** (found 2026-09-01, axe sweep of all fifteen signed-in routes in both themes). `text-ink/55` — with `/50` used interchangeably — is *the* secondary tone: tracked all-caps labels, metadata lines, column headers, `‹ BACK` links. ~460 nodes across 81 files, measuring **3.92:1** on cream against AA's 4.5:1, at 9–10.5px, the smallest type the app ships and the type it ships most of. Light-ledger-only: the same opacity resolves to 5.4:1 on the dark ground, because an opacity fade loses contrast fastest over a light one. Fixed by naming the tone instead of blending it — `--color-ink-muted`, `#6b6861` light (4.9:1) and `#8e8981` dark (**exactly what `/55` already resolved to**, so dark is unchanged), swept mechanically. Guarded by `src/styles/__tests__/contrast.test.ts`, which reads the real stylesheet. Also raised individually: the **tab bar's inactive labels** (`ink/45`, 2.9:1 — primary navigation, on every screen, the worst contrast measured), the `/more` version link, and the planner's `RIR` line and `MEV · MRV` band. Left as fades where the dimness *is* the message (unplanned blocks, skipped rows, the untouched-RIR assumption per doc 21 §6.1, placeholders, `aria-hidden` glyphs). Design record: [`09-changelog`](../09-design-changelog.md) 2026-09-01 | B | MED | W | **done (PR #TBD)** |
| N93 | **`/sign-in` threw a hydration error on every visit** (found 2026-09-01). React error #418 — *"the server rendered HTML didn't match the client"* — fired on the sign-in page's first paint, so React **discarded and re-rendered the whole form** and the app's own client-error reporter logged it. Cause, from the dev-mode diff: `<Suspense><RedirectField/></Suspense>` was the **first child** of `<form action={formAction}>`, and React injects its own hidden `$ACTION_REF_1` input into that exact slot during SSR. Fixed by moving the boundary below the two inputs — position is irrelevant to a hidden field's form data. Verified: a full sign-in → sweep of ten routes now reports no page errors at all | B | LOW | G | **done (PR #TBD)** |
| N94 | **A disabled action's label was unreadable, and in dark mode it outranked the enabled one** (found 2026-09-01). The planner board's save button in its `NO CHANGES` state is `bg-ink … disabled:opacity-40`, which fades the label *with* the fill: roughly **1.7:1** of label against its own background, in both themes. That label is the reason the action is off. Dark was worse than unreadable — the faded cream fill became the brightest block on screen, so the unavailable action outranked the available `CANCEL` beside it. Fixed narrowly: that button hollows out (`border-ink/25` + muted label, matching `CANCEL`'s border so the height doesn't jump between states). **Deliberately not swept** across the ~30 other `disabled:opacity-40` buttons — nearly all are *pending* states during a submit, where a brief fade is the right cue; only a standing "nothing to do here" needs its label | UX | LOW | W | **done (PR #TBD)** |
| N95 | **The accent as small text measures 4.28:1 — an owner call on the brand hex.** The last contrast failure standing after N92. `#C14B2A` on cream is 4.28:1 where AA asks 4.5, on `TARGET n RIR`, the `CURRENT` / `ACTIVE` badges, `DELETE ›`, `IN PROGRESS` and `BALANCE CHECK` — all small bold text, none of it large enough for the 3:1 large-text allowance. A ~5% shortfall: darkening to roughly `#BB4828` clears it and is imperceptible side by side. Not done in N92's pass because `#C14B2A` is the palette doc 08 §1 fixes, and repainting a documented brand colour is not a change to make from an audit. Note the accent's **fill and border** uses (the position marker, badge borders, the progress bar) are fine as they are — non-text contrast asks only 3:1, so this is a text-only question. Options: (a) darken `--color-accent` in the light theme only; (b) add a separate `--color-accent-text` for the small-text uses and leave the fill hex alone; (c) accept 4.28 and record the exception | D | LOW | W | **needs-input** — owner decision on the doc 08 §1 accent |
...
| N88 | **The mesocycle seed treated an exercise with years of history as brand new** (owner, 2026-08-15). *"Kneeling Hamstring Curl has plenty of logged history, but when the current mesocycle was created, the prescription engine incorrectly treated it like a brand-new exercise with no history… it failed to create a strength anchor and left the starting weight blank… This is not because the history is too old."* **The owner's read was right, including the part ruling out age.** `getExerciseE1rmAnchors` (`queries/anchors.ts`) fetched every exercise's candidate sets with ONE recency-ordered query bounded by a global `.limit(600)`. That is a **per-call** cap, not a per-exercise one: the rows returned are the *batch's* 600 most recent, so an exercise on a longer rotation than its batch-mates can have its whole history evicted by **other** exercises' recent sets, come back empty, and seed a blank weight. **The bug is a function of batch width, not of age** — which is why it bites the meso seed (one call, every exercise in the plan), spares the single-exercise paths, and why re-seeding the same exercise alone silently "fixes" it. Reconstructed against live data for the 2026-08-10 seed of *August '26 - Bulk* (23 exercises): **815 eligible sets sat newer than Kneeling Hamstring Curl's most recent set**, so all 66 of its rows fell outside the cap (best rank 755), as did all 52 of Barbell Hip Thrust's (best rank 2860). Those two, and **only** those two, recorded `strengthAnchor: null` — no false positives, no false negatives. Hip Thrust is the second exercise the owner remembered: it was re-seeded alone on 08-12 and immediately anchored at 286.7 lb off the same untouched history. The irony is in the module's own comment, which argued *against* a `performed_at` recency floor (~56% of pairs are >4 half-lives stale) and then closed with *"egress is already bounded by the .limit(600) below"* — a global LIMIT over a recency-ordered union **is** that floor, at a cutoff that moves with batch size. It also silently falsified manual claim `C-wt-04` (*"a lift you last trained months ago still has an anchor"*). | B | **HIGH** | K | **done (PR #250)** — the bound is now **per exercise**. New view `v_anchor_candidate_sets` (migration `20260815000001`) ranks each user's candidates *within* an exercise (`row_number() partition by user_id, exercise_id order by performed_at desc, id desc`); the caller asks for `set_rank <= 40`, which Postgres applies as a WindowAgg **Run Condition** — it stops early per partition, so egress stays bounded without a global cutoff, and the existing `logged_sets_user_exercise_idx` serves the partition + order exactly (verified by EXPLAIN: index-cond pushdown, 0.8 ms). **Eligibility moved into the view** — non-warmup, rep-bearing, and the N3 completed-workouts-only rule — so a rank slot is never spent on a row the caller would discard, and that also deletes a round-trip. `id desc` breaks ties because imported history shares one `performed_at` across a whole session and an unstable sort would truncate a session mid-way, handing the engine a partial `session_best`. Regression test is an **integration** test (`tests/integration/anchor-batching.test.ts`) on purpose: the defect was in SQL semantics, so a fake client would have to reimplement the ranking and assert against itself. **Two follow-ons the fix does not cover, and they run in OPPOSITE order:** the migration must reach hosted **before** the deploy (the code reads the view) — **applied 2026-08-15**, view verified (Kneeling Hamstring Curl now ranks at 1 with 40 rows inside the cap; advisors clean) — while the re-seed of already-blanked prescriptions must come **after** it, because the MCP connector runs deployed code and recomputing early would re-derive the anchor through the old global-limit query and write another null. Runbook item ① carries both. |
| N89 | **Bring the `Load Weights` Apple Shortcut into the app as a plate loader** (owner, 2026-08-30, with the shortcut's own spec attached). *"I have an apple shortcut that does this. I would like to add this as a tool to the workout app… place it in the exercise drop down menu as a tool for loading plates… brought up in a bottom tray, and then execute its quick multi-stage workflow in pages which advance right to left with each step… swipe to go forwards and back… The weight should be autofilled from the weight of the active set of exercise it was selected from, but allow the user to change it (changing it from load plates would update the set too)."* The shortcut's arithmetic is small and fully specified: subtract the unloaded implement, split across one or two sides, spend `[45, 25, 10, 5, 2.5]` greedily largest-first without exceeding the ask, report the closest achievable total and the count per side. **The interesting half is the coupling, not the math** — a plate calculator that does not know the set makes you type the weight twice and lets the two disagree, so the tray opens on the ACTIVE set's weight and a change there writes back through the same queued `plan_weight` op a weight-cell blur uses (`Match weight across sets` therefore governs its fan-out unchanged). **No mockup exists**, so hard rule 8 was discharged as a design pass rather than a transcription (09-changelog 2026-08-30). Two calls worth restating: the bar weight + loading points live in **device storage** per exercise, not on the account — they describe the rack you are standing at, must work with no connection, and the equipment defaults are only an opening bid; and the row joins the N82 *look it up* group **second**, the one insertion that moves no existing row under a returning thumb. Absent on a bodyweight-only movement. | F | — | E | **done (PR #255)** — released in **v1.2.0** (production cut 2026-08-30); the `releaseActive("1.2.0")` gate is now live. `src/lib/plates.ts` (pure, 21 tests incl. the shortcut's own 185 lb example and a float-drift case) + `PlateSheet.tsx` (four-page tray, swipe + `Next`/`Back`) + one gated menu row. Guide: ch. 5 `#adjusting-as-you-go` gains *Loading the bar*, claims `C-plate-01`…`05`. **Round 2 (owner, same session):** the note's copy is now the owner's own plain sentence, and it carries a **recording** — which needed a new release-note media pattern (`ReleaseEntry.media`, one renderer, CI gates on path/size/alt, its own SW cache, capped at 260px, one theme) before it needed an asset. The clip is recorded against the real components in a production build, not re-drawn; 09-changelog 2026-08-30 §7–§8. |
| N90 | **The prescription strip contradicted the trace, the anchor, and the history sheet at once** (owner, 2026-08-23, with three screenshots). *"There appears to be an issue with disagreement / inaccuracy between the prescription, the prescription detail, and reality. I have noticed this before."* Kneeling Hamstring Curl, W2·D4 of *August '26 - Bulk*: the strip read *"Versus last session: up 10 lb, 3 more reps per set… The weight goes up because you completed last session's target in full"*, while the details sheet directly beneath it read `LOAD — hold 40 lb` with `MEASURED ANCHOR 53.3 lb · 40 × 8 on 16 Aug`, and the History sheet read `40 lb × 8, 8, 8`. **Both wrong lines came from the explanation layer, not the engine** — the numbers were right the whole time. **Cause 1 (the baseline):** `composeDelta` used `inputs.previous`, the previous *prescription*, as "last session". The engine does not: `assessPerformance` reduces `inputs.actualSets` to the best working set and prices off *that* (`baseWeight`), which is why the trace said hold. The seed had asked for 30 × 7 (a leftover of **N88**); the owner loaded 40 and did 8, 8, 8; the strip then reported the gap between two prescriptions as if it were the lifter's own week. Any session loaded off-prescription reproduces it. **Cause 2 (the copy):** `composeProgressionLine`'s `stepped` branch was hardcoded to *"The weight goes up"*, but an earned step is a target **strength** (doc 16 §3.3) and the load rule is free to spend it on reps at a held weight — as it did here (8 → 10 at 40 lb). **Same root in the coaching layer:** `explanation-facts.ts` labelled the previous prescription `previous_work` and handed it to the model as performance, and `projectChange` called this decision `load_increased`. | B | **HIGH** | P | **done (PR #258)** — one baseline, shared. New leaf `src/lib/engine/best-set.ts` holds the single definition of the set that counts; `assessPerformance` and the new `readPerformedWork` (`queries/audit.ts`, off the decision's own `inputs.actualSets`) both use it, with a test asserting the two never drift. The delta now splits its baselines deliberately: the **work** axes (weight, reps) compare against what was performed — so the strip structurally cannot contradict the trace's `hold` — while the **program** axes (set count, effort target) still compare target to target, because a set left unfinished is not the program dropping one. `stepped` branches on where the step actually landed, borrowing the `paced` line's construction when the weight held. The facts layer gets the same split, so `previous_work` is work. A ragged session drops the phrase "per set" and compares against the best set by name. Decisions with no recorded actuals keep the old target-to-target reading, which is all they can support. |
...
### Batch 38 — mesocycle seeding missed existing history (2026-08-15, session task)

> "Please investigate the following issue with my latest seed prescription for
> the upcoming Kneeling hamstring curl session:
>
> Kneeling Hamstring Curl has plenty of logged history, but when the current
> mesocycle was created, the prescription engine incorrectly treated it like a
> brand-new exercise with no history. Because of that, it failed to create a
> strength anchor and left the starting weight blank.
>
> This is not because the history is too old. Old exercise history is still
> supposed to be usable. The issue appears to be that the mesocycle seeding
> process failed to retrieve the existing history for this exercise.
>
> This happened on another exercise earlier this week which also had history,
> but seeded no anchor. This seems like a bug and doesn't match the intended
> behavior. Assess and address"
>
> *[→ **N88**, fixed in PR #250. Confirmed as a real defect and diagnosed
> against live data: the anchor query's `.limit(600)` was a **per-call** cap
> over a recency-ordered read spanning the whole batch, so a longer-rotation
> exercise's history was evicted by its batch-mates' recent sets. The owner's
> two exclusions were both correct — not age (the module has no recency floor
> by design), and yes a retrieval failure. The "other exercise earlier this
> week" was **Barbell Hip Thrust**, which self-healed on 08-12 when it happened
> to be re-seeded alone, in a batch small enough for the cap not to bite.]*

### Batch 39 — the plate loader (2026-08-30, session task)

> "I have an apple shortcut that does this. I would like to add this as a tool
> to the workout app. Please build it and place it the exercise drop down menu
> as a tool for loading plates.
>
> The tool should be brought up in a bottom tray, and then execute its quick
> multi-stage workflow in pages which advance right to left with each step. The
> user should be able to swipe to go forwards and back in the workflow too. The
> weight should be autofilled from the weight of the active set of exercise it
> was selected from, but allow the user to change it (changing it from load
> plates would update the set too), then the user advances through the workflow
> and quickly and gets what plates to load. Style it in keeping with the app
> themes.
>
> This should be staged as a feature update fyi."
>
> *(attached: `Load Weights — Apple Shortcut Implementation Spec`, the
> shortcut's inputs, calculation flow, plate-selection loop and worked 185 lb
> example, closing with "the available plate-size array should ideally be
> configurable rather than permanently hard-coded".)*
>
> *[→ **N89**, built the same session. The math is a faithful port
> (`src/lib/plates.ts`), kept pure and given the rack as a **parameter** rather
> than a constant, which is the shortcut spec's closing note honored as far as
> it can be honored without a UI nobody asked for yet. Three things the port
> adds because it is in the app and not beside it: the tray opens on the active
> set's weight and writes a change back to it; the bar weight and loading points
> are remembered per exercise so the middle two pages are a tap each; and an
> unreachable ask is *stated* — the closest total under it, how far short, and
> an offer to record that number — rather than silently rounded. Staged behind
> `releaseActive("1.2.0")` per the owner's closing line.]*

### Batch 40 — the prescription strip disagreed with its own trace (2026-08-23, session task)

> *[three screenshots: the day view's prescription strip for Kneeling Hamstring
> Curl, the Prescription details sheet for the same row, and the History sheet —
> with "up 10 lb, 3 more reps per set" and "40 lb × 8, 8, 8" highlighted]*
>
> "There appears to be an issue with disagreement / inaccuracy between the
> prescription, the prescription detail, and reality. I have noticed this
> before. Please assess and address"
>
> *[→ **N90**. Both disagreeing lines were the explanation layer's, not the
> engine's — every number on the sheet was already correct. The delta line and
> the coaching facts read the previous *prescription* as "last session" while
> the load rule prices off the previous session's best **working set**; on this
> row the two differed (the N88 seed asked 30 × 7, the owner did 40 × 8 × 3) and
> the strip narrated the gap between two prescriptions as the lifter's week.
> Separately, the `stepped` copy claimed a weight rise for every earned step,
> including the ones the load rule spends on reps at a held weight — which is
> what happened here. "I have noticed this before" is consistent with the
> cause: it fires on any session loaded off-prescription.]*
