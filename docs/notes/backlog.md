...
| R9 | `analyze_exercise_progress`: any phase with ≤3 sessions reads "improving", even a strict decline (every phase start asserts improvement) | B | MED | G | **done (PR #117)** — short phases (no prior baseline) now read the trend within the window (latest vs first, tolerance-banded) instead of asserting improvement; new trend tests incl. the strict-decline + two-point-drop cases |
| R10 | Replay re-runs seed decisions without `bodyweight` → every bodyweight-lift seed diffs spuriously, corrupting the params tuning loop | B | MED | G | **done (PR #117)** — replay's seed branch passes the stored `bodyweight`; regression test proves a bodyweight seed replays unchanged under v16 (fails without the fix) |
